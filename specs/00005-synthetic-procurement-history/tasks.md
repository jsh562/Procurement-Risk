...
- **No DDL and no migration.** E003 owns both tables and they are delivered; this epic writes rows only. E005's reserved filename block `0200`–`0299` is claimed and expected to go unused, as are decision-record numbers from `0018`. **No task in this list authors a migration**, and a task that would is out of scope rather than missing. *(Corrected 2026-07-28: the decision-record half is false — `0018` went to E007 and `0019`–`0021` to E006. See `spec.md` § Implementation Signals. Nothing about this epic's task list changes; the numbers were never used here.)*
...
- **~~Gated, and deliberately unimplemented~~ — DISCHARGED 2026-07-26, corrected here 2026-07-27**: **FR-034 and SC-026** required manufacturer and part-number fields E002 did not publish *at the time this line was written*. E002 published both on 2026-07-26 and the gate discharged. T026, T027 and T029c implement them and `equipment.py` delivers them. The prohibition — "no task attempts to satisfy them, **and none may**" — was removed from `data-model.md` when the gate discharged and survived here for a further day: the half that says *this cannot be done* outliving the half that says *this is not done*, which is the lesson `plan.md` § Compliance Result already records. T077 is inverted and now guards against the fields being withdrawn.
...
- [X] T018 [P] [US1] {FR-036} [COMPLETES FR-036] **NC-6** — an unadjusted ratio in band whose category-adjusted ratio is outside it must fail, in TST/test_spread_band.py after:T017
- [X] T019 [P] [US1] {FR-009} **RED** Failing censor property tests in TST/test_censor_properties.py — no instant past as-of, event 1 equals `order_date`, monotone delivered set (DV-008)
- [X] T020 [US1] {FR-009,FR-010} **GREEN** Implement the committed window and as-of truncation with DV-010's three floors asserted as one window in PKG/censor.py after:T019
- [X] T021 [P] [US1] {FR-010} **NC-5** — event-floor, censoring-floor and empty-non-terminal-state breaches all exit non-zero, no artifact written — TST/test_shape_floor_controls.py after:T020
- [X] T022 [P] [US1] {FR-011,FR-012} **RED** Failing criticality property tests in TST/test_criticality_properties.py — all nine cells, tercile ties at a cut point, zero slack (DV-006, DV-013)
- [X] T023 [US1] {FR-011,FR-012,FR-035} **GREEN** Implement multiplicative slack, need-by, the pressure terciles and the tier×tercile table in PKG/criticality.py after:T022
- [X] T024 [US1] {FR-005,FR-006} Implement the legal transition walk and the declared rework allocation (`6+3L` events, new positions) in PKG/lifecycle.py ← T014:line_generator
- [X] T025 [US1] {FR-005,FR-006} Assert DV-007 (contiguous sequence, first event `submitted`, every pair legal, no position reused) and DV-009 (cap 3, and the realized looped-line count and one/two/three histogram **equal** the declared `L = round(0.30 x N)` and `(42, 13, 5)` — equality, not recording) in TST/test_lifecycle.py after:T024
- [X] T026 [US1] {FR-031,FR-032,FR-034,FR-037} **RED** Write the failing property tests for the description grammars, the four-clause overlap predicate, and the **catalog draw** — every drawn `manufacturer` is a catalog `canonical_name` and never an alias, its `categories` contains the line's `material_category`, `part_number` carries that same entry's `part_number_prefix` and matches E002's `PART_NUMBER_PATTERN`, and the complement leaves both `NULL` — in TST/test_equipment_properties.py. `equipment.py` is the seventh mandatory deterministic-computation module, so its test commit MUST be observed failing first ← T014:line_generator
- [X] T027 [US1] {FR-031,FR-032,FR-034,FR-037} **GREEN** Implement the description grammars, the four-clause predicate, and the catalog draw in PKG/equipment.py — manufacturer and part number read through `model.corpus.manufacturers.load_catalog()` and `manufacturers_for_category()`, never restated in source. Then assert DV-004 (six non-blank descriptive fields, category a map key), DV-005 (fixed scale 1, UoM domain) and DV-014 in TST/test_equipment_properties.py and TST/test_overlap_predicate_control.py after:T026
- [X] T028 [P] [US1] {FR-032} [COMPLETES FR-032] **NC-7** — every complement line fails all four clauses, so the share can fall below 60% — TST/test_overlap_predicate_control.py after:T026
- [X] T029 [P] [US1] {FR-037} DV-021 — no `manufacturer` enters E001's real-firm exclusion list or matches its vendor convention, via `roster.naming` — TST/test_manufacturer_exclusion.py after:T026
- [X] ~~T029a~~ **withdrawn 2026-07-27 — folded into T026.** Added to close audit finding C-004 ("FR-034 is live with no implementing task") when T027 already covered the draw; C-015 had just reworded T027 from "disjoint manufacturer space" to "catalog-drawn" and the pre-edit meaning was read when adding this. Its assertions are T026's. Withdrawn rather than deleted and **not renumbered**, so T030+ keep the identities other artifacts cite
- [X] ~~T029b~~ **withdrawn 2026-07-27 — T027 implements it in `equipment.py`.** A separate `manufacturer.py` would have made an eighth mandatory property-tier module and forced the seventh-module counts to be re-propagated across `plan.md` and `tasks.md` — which is exactly the half-propagation that produced finding C-010
- [X] T029c [P] {FR-034} DV-028 — assert the catalog-overlap share is ≥ 60% over all lines and record the realized share in the ground-truth record, **separately from FR-032's share**: the two predicates are scored by separate functions and reported as separate figures — TST/test_generate.py::TestTruthRecord and TST/test_overlap_predicate_control.py after:T027
...
- [X] T031 [P] [US1] {FR-017} DV-017 — exactly 12 unique `vendor_id`s covering the roster and a `dataset_content_hash` equal to the fixture's — in TST/test_generate.py::TestTruthRecord after:T030
...
- [X] T050 [US2] {FR-027} **NC-4** integration half — a mutated roster and a mutated category map each refuse the load naming that input (DV-016) — TST/test_load_refusal_controls.py::TestInputDriftRefusal after:T042
...
- **Mandatory red-green pairs** (`plan.md` §The test-first observable — the seven property-tier modules; six of the seven are evidenced in history, see the validation table): **T006 before T007** (`serialize.py`), **T010 before T011** (`allocate.py`), **T013 before T014** (`seeds.py`), **T015 before T016** (`durations.py`), **T019 before T020** (`censor.py`), **T022 before T023** (`criticality.py`). The test task must be observed **failing** before its implementation task begins, and the branch history must carry a `test:` commit before the `feat:` commit for each pair.
...
- **Reporting obligations carried from `plan.md`**: A1/A2 at T078 (denominators derived by counting IDs — 33 criteria, 37 requirements), A3 at T079 (`WITHDRAWN` retired unused, no stale `BLOCKED` string survives), A4/NC-12 at T077 (inverted — the corpus fields are present and must not silently disappear).
...
| Check | Result |
|---|---|
| Task IDs contiguous from T001 | **86 task lines** — T001–T083 contiguous, plus T029a–T029c appended when FR-034's gate discharged. T029a and T029b are withdrawn and struck through rather than renumbered, so T030+ keep the identities other artifacts cite |
...
| Every DV rule implemented or asserted, at the tier `data-model.md` assigns it | **28 / 28** (DV-001…DV-028; DV-028 added with FR-034's design when the gate discharged) |
| Every negative control lands as a task | **12 / 12** (NC-1…NC-12, rendered as 13 rows since NC-11 split into 11a/11b) |
| Mandatory red-green pairs present and correctly ordered | **6 / 7** — six pairs carry a `test:` commit before their `feat:` commit in the branch history (serialize, allocate, seeds, durations, censor, criticality). **The seventh does not**: T026 (RED) and T027 (GREEN) for `equipment.py` both landed inside one commit, so the process obligation `plan.md` § The test-first observable states — *the branch history carries a `test:` commit before the `feat:` commit for each pair* — is unevidenced for the module A-005 promoted to the mandatory tier. History cannot be rewritten to manufacture the evidence, and doing so would be the simulation the execution policy forbids. Recorded as a real miss rather than closed |
