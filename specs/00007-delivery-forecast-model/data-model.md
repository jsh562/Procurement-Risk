...
| Not computed in the database | Every probability, quantile, digest and diagnostic is computed in Python inside the deterministic-computation boundary ({SAD:ADR-0008}, Principle V, FR-024). The database stores results and enforces shape. The one exception is E010's read-side `survival[d - as_of_date]`, which is E010's SQL and not E007's. **No complement** — `survival[k]` is already the probability that delivery has not occurred by day `k`, which is the probability of lateness itself. An earlier revision of this line carried `1 - survival[...]`, copied from E003's data model while that document was wrong; it was corrected on 2026-07-28. The grid this epic writes was always right: `survival_grid` computes `count(draws > k)/draw_count`. |
...
| Read by | E010, as `survival[d - as_of_date]` — no complement | E014 only. **E010 must never read this table** — it has no way to distinguish anchors, which is the entire reason the two populations are apart. |
...
| 4 | E010's read contract `survival[d - as_of_date]`, true of every row in the table it reads | It is *not* true of `held_out_prediction`. "The worklist reader must never read the held-out store" is a documented contract with no schema mechanism — ADR-0018 § Consequences/Negative |
...
| DV-019 | A draw-digest mismatch is reported as a scope limit, not a failure, **in every case**, and the reported scope limit names which of two readings produced it: the observed environment differs from `library_versions`, or it matches the whole recorded set and **that set does not determine bitwise numerics**. No disposition of the claim reaches the exit status; SC-018's day tolerance is the only gate. **An earlier revision made the scope limit conditional on a differing library version**, which implies an in-pin mismatch is a failure and is how it was implemented — and that is wrong for the reason **G-21** records, which is an observation and not a mechanism: on Linux an in-pin re-fit moved all 68 lines' digests while agreeing to 0.12 days against a 5.0-day tolerance, so a mismatch is *observed with every recorded key equal* and the pin therefore does not determine the digest. **Why it does not is unestablished** — sampler determinism at a fixed seed, model-construction determinism across a rebuild, and float64 fidelity through the Postgres round-trip were each measured and each held bitwise — so this rule rests on the observation and names no cause. The pin decides the *reading*, never the disposition. | Test | Unit |
...
| G-21 | **`forecast_run.library_versions` is measured not to determine the stored draw digests, so the recorded manifest cannot say whether two runs sampled in the same numeric environment.** The pin is the six package versions `ck_forecast_run__library_versions_shape` requires — `pymc`, `arviz`, `numpy`, `pandas`, `pytensor` and `blas` — and it is complete for what it records. **The gap is an observation, not a mechanism.** On Linux a re-fit of one recorded run, in the same process, at that run's own seed and shape and with every one of those six keys equal, moved every one of 68 lines' `draw_digest` values, while the largest realized median drift was 0.12 days against AD-004's published 5.0-day tolerance; on Windows the same comparison moves nothing. A mismatch is therefore reported *with the pin matching*, which is sufficient on its own to justify degrading the claim. **What causes it is unestablished**, and three candidates are ruled out by measurement on the CI Linux image against the real database — recorded here so the next reader does not repeat the work: **(i)** sampler non-determinism — sampling twice from one built graph at one seed returns all ten posterior variables bitwise identical; **(ii)** graph-construction non-determinism — rebuilding the model and resampling returns the same, bitwise; **(iii)** loss in storage — a 4005-value `float64` array including subnormals, `nextafter(1, 2)` and `1/3` survives the Postgres round-trip bitwise, worst delta `0.0`. **An earlier revision of this row named multi-threaded BLAS reduction order, instruction-set dispatch and scheduling as the cause. Nothing measured supports that, and it is withdrawn** — it was written from what the failure ought to have been rather than from an experiment, the same defect class this epic records against six published bounds | A column can hold a value the job can read, and there is no known value here to read: the properties that *would* determine the digest are unidentified, which is precisely the gap. A fingerprint chosen before the mechanism is known is worse than none — it would license exactly the inference this gap exists to refuse, that a matching fingerprint means matching bits, while never having been shown to determine them | Recorded as uncovered rather than presented as enforced, and the requirements corrected to match: **FR-032**, **SC-030** and **DV-019** now make every digest mismatch a reported scope limit and take the claim out of the exit status, so nothing in the epic infers a numeric environment from the pin. The pin keeps its job — it decides which of two *reasons* the scope limit reports, and `library_versions` remains a field FR-022 requires **exactly** equal, so an out-of-pin re-fit still fails the reproduction on the provenance ground. **The consequence is bounded rather than dismissed**: on the evidence available the difference is confined to this one optional claim, because FR-022's day-tolerance verdict agrees on both platforms with three orders of magnitude to spare — that is a statement about what was measured, not a general claim that differing bits are harmless. **Reversal trigger**: an environment fingerprint that is *shown* to determine the stored digest — which requires **finding the real mechanism first**, since the fingerprint's fields are whatever that mechanism turns out to depend on, and one proposed before it is known cannot be demonstrated to determine anything. The demonstration owed is the one this gap failed to obtain: on the platform where the digests currently move, two re-fits of one recorded run with the candidate fingerprint held equal, digesting equal. At that point an in-pin digest mismatch becomes a genuine failure and this gap closes. **Production-scale alternative**: bisect the divergence to its source before pinning anything around a guess — the three probes above narrow it, and the next steps are comparing the two runs' draws element-by-element rather than by digest to see where they first part, then repeating the comparison with the sampler's parallelism and the process's thread counts each held fixed in turn. A fit executed in a digest-pinned image is worth having on its own terms and is the natural place to *hold* such a fingerprint once one exists, but until the mechanism is known it makes the environment repeatable without establishing that what is recorded is what decides the bits |
