...
| `uq_threshold_calibration__strata_thresholds` | `UNIQUE (estimation_stratum_hash, hard_negative_stratum_hash, merge_threshold, reject_threshold)` | Redundant against the key above by design. It is `resolution_run`'s FK target, and it is the four columns that make **SC-040** structural: a run declares its two stratum hashes and its two thresholds in one referencing tuple, and a run whose thresholds differ from the calibrated pair for those hashes **has no referent and cannot be inserted**. The run refuses rather than publishing, and it refuses at the first write. |
...
| `fk_resolution_run__calibration` | `(estimation_stratum_hash, hard_negative_stratum_hash, merge_threshold, reject_threshold) → threshold_calibration` `MATCH FULL ON DELETE RESTRICT ON UPDATE CASCADE` | **FR-044 and SC-040.** The run's thresholds must be the ones calibrated against the labeled-set hashes it names. Post-hoc tuning is not detected by an audit afterwards — it fails on the first `INSERT`, before a single candidate pair exists. |
...
| `figure_name` | `text` | NOT NULL | part of the PK; `ck_resolution_figure__name CHECK (figure_name IN ('merge_precision','recall','blocking_pair_completeness','coverage','reduction_ratio','withheld_share'))`. The six figures the spec names, as a closed set — so **SC-027's "every published figure resolves to the manifest"** is the primary key, and a seventh figure invented at report time has nowhere to live. |
| `figure_kind` | `text` | NOT NULL | `ck_resolution_figure__kind CHECK (figure_kind IN ('estimate','census'))`, **and** `ck_resolution_figure__kind_matches_name CHECK ((figure_kind = 'census') = (figure_name IN ('coverage','reduction_ratio','withheld_share')))`. **FR-038's classification as a database fact.** The three census figures cannot be recorded as estimates and the three estimates cannot be recorded as censuses, so the interval rules below cannot be routed around by mislabelling a figure. |
...
| `target_value` | `double precision` | NULL | NULL where no target is registered. Merge precision's 0.95 and recall's 0.80 come from `specs/prd.md`; the other four have none. |
...
| 10 | `INSERT resolution_figure` — all six | After every population they are computed over exists. Six rows, always: VR-013 asserts the count. |
...
| `0501` | `0501_labeled_set.py` | `0500` | `labeled_pair_set`, `labeled_pair`, `threshold_calibration`, and `uq_labeled_pair_set__hash_stratum`, `uq_labeled_pair__set_pair`, `ix_labeled_pair__pair_identity`, `uq_threshold_calibration__strata`, `uq_threshold_calibration__strata_thresholds` | Independent of `0500`; ordered after it only to keep one linear chain. `threshold_calibration` cannot be split from `labeled_pair_set` — its two foreign keys reference `uq_labeled_pair_set__hash_stratum`, and a split leaves an intermediate head at which the calibration has no referent. |
| `0502` | `0502_resolution_run.py` | `0501` | `resolution_run`, its three redundant unique keys, `ix_resolution_run__single_active`, `ix_resolution_run__started_at`, and `v_active_resolution_run` | After `0500` and `0501` — both foreign keys resolve there. The view reads the table, so it cannot be split from it. |
...
| `0508` | `0508_resolution_figure.py` | `0507` | `resolution_figure` and its checks | After `0502`. Last of the object revisions. |
...
| `threshold_calibration` | table | `0501` | The committed thresholds, bound to the two stratum hashes they were calibrated against |
...
| `resolution_figure` | table | `0508` | The six published figures, with the interval rules as row constraints |
...
| `uq_threshold_calibration__strata_thresholds` | unique index | `0501` | FK target making SC-040 structural |
...
| `uq_threshold_calibration__strata_thresholds` | unique | `(…, merge_threshold, reject_threshold)` — SC-040's FK target |
...
| `fk_resolution_run__calibration` | foreign key | `(estimation_stratum_hash, hard_negative_stratum_hash, merge_threshold, reject_threshold)` → `threshold_calibration`, `MATCH FULL ON DELETE RESTRICT ON UPDATE CASCADE` — **FR-044, SC-040** |
...
| `ck_resolution_figure__name` | check | `figure_name IN ('merge_precision','recall','blocking_pair_completeness','coverage','reduction_ratio','withheld_share')` |
...
| `ck_resolution_figure__meets_iff_target` | `target_value`, `meets_target` | Both null tests. Four of the six figures have no registered target and carry NULL in both. |
...
| 12 | **A run's thresholds are the ones calibrated against the hashes it names** | `fk_resolution_run__calibration` against `uq_threshold_calibration__strata_thresholds` — a tuned run has no referent (FR-044, SC-040) | composite FK |
...
| VR-013 | A completed run has **all six** `resolution_figure` rows. Asserted by counting against the `ck_resolution_figure__name` domain read out of `pg_constraint`, so adding a seventh figure to the vocabulary without publishing it fails the build rather than being silently unpublished. Separately: no figure carries `stratum = 'hard_negative'` — asserted by attempting the insert and expecting `ck_resolution_figure__stratum`, since a domain member that is absent by omission needs a test that observes the omission. | `resolution_figure` | FR-025, FR-037a, SC-027, SC-037 |
...
|---|---|---|---|---|
| **A-1** | US1 acceptance scenario 7 vs SC-001, US1's narrative, FR-041, and the fourth-pass clarification | Scenario 7 requires that a resolved entity "asserts no association to a specification section, and the run's report states that exclusion with its cause". SC-001 requires a triple whose specification member is "sourced from an extracted `specification_section` value", and the fourth-pass clarification withdraws the exclusion on verified evidence. **These cannot both be satisfied by one run.** Scenario 7 is residue of the third pass, which the fourth pass reversed and did not delete | **The schema decides neither.** `ix_rem__single_specification_section` bounds the count at one and requires none, so a cluster with a specification member and a cluster without one are both storable. Which a run should produce is not a schema question | If scenario 7 governs, `member_role`'s `'specification_section'` value becomes unused and the partial index becomes an unexercised guard — worth deleting, and worth *knowing* is unexercised. If SC-001 governs, nothing changes |
| **A-2** | FR-045 | FR-045 names `uq_rem__extracted_value` and `uq_rem__po_line` as the constraints needing run scope, and does not mention `uq_resolved_entity__normalized_identity` — which is equally unbuildable under FR-039, because run 2 emitting an entity for the same manufacturer and part number collides with run 1's | Re-scoped on the same authority, to `uq_resolved_entity__run_identity (resolution_run_id, normalized_manufacturer, normalized_part_number)`. The requirement's enumeration is read as incomplete rather than as exhaustive | If the enumeration is exhaustive, FR-039 is unbuildable and the epic cannot ship — so the reading is forced, but the requirement should say so |
...
| `resolution_figure` | Exactly six per run | Free. |
...
| `threshold_calibration` | The committed thresholds, bound to the two stratum hashes they were calibrated against | `0501` | E009 |
...
| `resolution_figure` | The six published figures, with FR-025/FR-027/FR-038's rules as row constraints | `0508` | E009 |
