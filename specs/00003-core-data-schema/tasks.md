...
**Input**: Design documents from `specs/00003-core-data-schema/`
...
- [X] T059 [BUG:WARNING] {TR-019,TR-061} [requirement-gap] SC-009 misattributes its second half to the G-5 test, which asserts the opposite — specs/00003-core-data-schema/spec.md
...
- [X] T060 [BUG:WARNING] {TR-084} [requirement-gap] SC-028 reads as an unconditional 100% claim but holds only for procurement_app — specs/00003-core-data-schema/spec.md
...
- [X] T061 [BUG:WARNING] {TR-037,TR-083} [requirement-gap] data-model.md TR-037 row and spec.md Scope-Excluded still describe the superseded Compose-job design — specs/00003-core-data-schema/
...
- [X] T062 [BUG:WARNING] {TR-042} [requirement-gap] tasks.md internal contradictions: inverted 0009/0010 edge list and a stale requirement count — specs/00003-core-data-schema/tasks.md:165
...
- [X] T064 [BUG:WARNING] {TR-063,TR-083} [requirement-gap] plan.md summary counts are stale against the delivered object set and gap list — specs/00003-core-data-schema/plan.md:37,102
...
## Phase: Post-QC Remediation (Analysis Findings A-007, A-008, A-011, A-012)

Generated 2026-07-26, after `.qc-passed`. These are **not** QC bug tasks — the gate was green before and after. They close the four analysis findings that had been deferred through five phases, examined for the first time against delivered code rather than against the spec as written. The examination changed the answer for the largest of them, so two of these tasks are decision records rather than edits. Full reasoning lives in `spec.md` § Compliance Check.

- [X] T067 {TR-078} [P] Correct the misattributed TR-078 citation and evidence the obligation it names — src/model/tests/schema/test_chunk.py
  > `test_deleting_a_document_with_chunks_is_refused` cited "TR-046, TR-078" while asserting `ON DELETE RESTRICT`, which is TR-046's direction. TR-078's obligation is `ON UPDATE CASCADE` (`data-model.md:725`), asserted nowhere. Same species as T066: a delivered test naming an ID whose obligation it does not evidence. TR-078 dropped from that docstring; `test_updating_a_document_key_cascades_to_every_chunk` added, asserting both the propagation and `pg_constraint.confupdtype = 'c'`.
- [X] T068 {TR-020} [P] Assert the purchase-order-line column set — src/model/tests/schema/test_procurement.py
  > `criticality` and `material_category` appeared only as fixture literals; nothing asserted the required columns exist. `test_a_line_carries_every_column_tr020_requires` asserts presence and `is_nullable = 'NO'` for the 8 facts TR-020 names, as a **subset** of the table's 21 columns — a closed set would report a legitimate later addition as a TR-020 violation.
- [X] T069 {TR-056} [P] Assert the seeded constant values — src/model/tests/schema/test_constants_agreement.py
  > Nothing asserted the seeded row holds 365, 4000, and 1e-9. The existing `test_published_tolerance_equals_the_literal_inside_the_residual_check` is **drift control, not value control**: it compares the row against the DDL literal and passes just as happily if both say `1e-3`. `survival_horizon_days` and `draw_count` have no DDL literal at all (`data-model.md` §Declared Constants records "none"), so before this test the seeded row was the only statement of those two numbers anywhere and nothing compared it to anything. All three values now asserted.
- [X] T070 {TR-062} [P] Assert the deliberate absence of per-line forecast lineage — src/model/tests/schema/test_forecast.py
  > TR-062 requires that no per-line link back to extracted values or lifecycle events exists. Asserted as an absence, with the forbidden edge defined as a relation referencing both a forecast relation and one of the four input relations. Deliberately does **not** include `chunk` or `document` in the input set — those are the citation chain behind an extracted value, and forbidding a reference to them would be a stronger rule than TR-062 states.
- [X] T071 {TR-080} [P] Assert the deliberate absence of a maximum forecast age — src/model/tests/schema/test_forecast.py
  > No max-age constant in `schema_constants`, no age or staleness column on `forecast_run`, and the anchor date exposed instead. The staleness threshold is E010's.
- [X] T072 {TR-053,TR-057,TR-062,TR-064,TR-077,TR-078,TR-079,TR-080,TR-081,TR-082,TR-085} Reclassify the eleven non-obligations phrased as MUST (A-011) — specs/00003-core-data-schema/spec.md
  > 11, not the 9 first counted. Each carries a trailing italic note naming what it actually is — hand-off, scope statement, semantic note, deliberate absence, or future-work policy — and where its enforceable half sits. A convention note was added to the Technical Requirements preamble. **No ID and no obligation changed**; verified by diffing the requirement-ID list against the committed version. TR-053, the one true duplicate in A-007, is marked subordinate to TR-030 in place rather than deleted, because its ID is live in three references — T044's task tag, a `plan.md` coverage row, and a `data-model.md` traceability row — and `artifact-conventions/SKILL.md` prohibits changing a requirement ID for exactly that reason. An earlier revision of this note justified it as "IDs are never renumbered", citing line 171 of this file; that line is an aside about **task** IDs and is not policy, and the blanket phrasing was wrong — a colliding ID is renumbered locally before it reaches the repository.
- [X] T073 {TR-063} Record A-007 as WILL-NOT-MERGE with the evidence — specs/00003-core-data-schema/spec.md
  > Decision record, no edit to any requirement. The finding claimed ~13 duplicate clusters over 46 IDs; re-examination found 15 over 51, of which **one** is a true duplicate. The other 14 are a general requirement plus genuine per-case specialisations, and each specialisation is the sole traceability anchor for its own named constraint — `ck_line_posterior__draws_sorted` (TR-070), `ck_line_posterior__survival_length` (TR-072), `fk_line_posterior__run_shape` (TR-073), `fk_purchase_order_line__closing_event` (TR-067), the migration-role grant (TR-086). Merging would leave those constraints tracing to nothing, which is worse than the redundancy. The original deferral reason was right for the wrong reason. **Re-confirmed by the user** after a second, bogus leg of this justification was withdrawn — an earlier revision claimed "the project already forbids the mechanism", which overstated `artifact-conventions/SKILL.md` and mis-sourced it to line 171 of this file. Will-not-merge stands on the traceability-anchor argument alone.
- [X] T074 {TR-056,TR-065,TR-076,TR-083} Record A-012 as accepted, with its mitigation and its carry-forward — specs/00003-core-data-schema/spec.md
  > A-012's text was **not recoverable from the working tree** — the superseding analysis pass renumbered to a B-series and overwrote `analysis-report.md` in place while three other files still cited the A-IDs, leaving four IDs with three descriptions between them. Recovered from `git show 7138026:specs/00003-core-data-schema/analysis-report.md`: four requirements make `data-model.md`, a Plan-phase artifact, normative over Specify-phase requirements, so a Plan re-run can invalidate a spec requirement. Accepted rather than corrected — the alternative lifts thirteen tables of DDL into `spec.md`, duplicating the normative source instead of removing the inversion. Mitigation already in practice: all four in-flight `data-model.md` amendments are labelled corrections carrying PostgreSQL 16 evidence and add no constraint name or object. **An ADR is required to bind this beyond E003 and must be authored on `main`** — carried forward with AR-1 and AR-2, not applied from this branch.
- [X] T075 {TR-078} Evidence TR-078's third clause end to end — src/model/tests/schema/test_extraction.py
  > T067 asserts the *mechanical reason* citations survive a key-space change (`chunk_id` unchanged across the cascade, that being the column `extracted_value.(source_chunk_id, cited_page)` references) but never inserted a citation and read it back after the rename. The inference is sound; it is asserted anyway, because three defects in this epic came from an implication that looked obviously true and was not. Placed in `test_extraction.py` because the vocabulary-seeded insert machinery lives there.

**Not actioned, deliberately.** A-008's 16 documentation gaps — a test asserts the obligation but no `SC-###` or validation criterion cites it. Adding 16 success criteria to an epic already measured 28/28 buys no verification and invites the same retroactive-adjustment concern Principle VII raised for SC-009 and SC-028. The gap stays on record in `analysis-report.md`. Also left alone: TR-065's three-rung fallback ladder contradicting the two-rung wording elsewhere (was B-007, never applied) — rung 1 was delivered and `data-model.md` records that no fallback was taken, so the unreachable middle rung costs nothing while editing it would touch a Technical Constraint, a validation criterion, a success criterion, and a plan risk row.
