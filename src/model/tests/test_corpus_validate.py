...
GENERATION_CONFIG = {"generator_id": "corpus-generate", "seed": 20260725}
...
def _build_pristine_corpus(root: Path, committed_root: Path) -> Path:
    ...
    # The three validator-read supporting artifacts are copied rather than
    # invented: the schema is the contract under test, and the policy and
    # ledger are what VR-022, VR-023, VR-025, VR-026 and VR-062 compare against.
    for relative in ("manifest.schema.json", "real/retrieval-policy.json", "real/exclusions.json"):
        shutil.copyfile(committed_root / relative, root / relative)

    # The three generator-read artifacts do not exist until Phase 5; they are
    # stubbed here because VR-065 closes the corpus root over exactly seven
    # files and because a SYNTHETIC entry's `generation_inputs` keys are fixed.
    (root / "synthetic" / "generation-config.json").write_bytes(_canonical_json(GENERATION_CONFIG))
    (root / "synthetic" / "equipment-category-map.json").write_bytes(_canonical_json({}))
    (root / "synthetic" / "field-label-vocabulary.json").write_bytes(_canonical_json({}))
    (root / "synthetic" / "datasheet.md").write_bytes(b"# Synthetic corpus datasheet\n")
    ...
    roster = read_roster()
    roster_hash = roster.content_hash
    # Keyed by the repository-relative literals VR-061 fixes, valued by a digest
    # over this tree's stand-in for each file. VR-061 itself is T042's; what
    # matters here is that the closed key set the schema requires is present.
    generation_inputs = {
        relative: sha256_of_bytes((root / "synthetic" / Path(relative).name).read_bytes())
        for relative in GENERATION_INPUT_PATHS
    }
    for project in roster.projects[:SYNTHETIC_PROJECTS]:
        directory = root / "synthetic" / project.id
        directory.mkdir()
        (directory / "doc-00.pdf").write_bytes(pdf)
        entry = {
            "location": "doc-00.pdf",
            "layer": "SYNTHETIC",
            "license_basis": {
                "basis_id": "project-generated-no-third-party-rights",
                "generated_by_this_project": True,
                "third_party_rights": "NONE",
                "statement": "Generated by this project; no third-party rights subsist.",
            },
            "content_hash": digest,
            ...
            "document_model_hash": OTHER_DIGEST,
            "irregularity_classes": [],
        }
        (directory / MANIFEST_FILENAME).write_bytes(
            _canonical_json(
                {
                    "location_id": f"synthetic/{project.id}",
                    "layer": "SYNTHETIC",
...
def test_the_committed_corpus_is_reachable_and_reports_populations() -> None:
    """The committed real layer, at the scope this phase's corpus is complete to.

    The synthetic layer lands in Phase 6 (T056); until then its populations are
    empty, which VR-005 and VR-066 are built to fail on. The run is therefore
    scoped to the layer that exists, and the scoping is visible in the report
    as SKIPPED rules rather than as silent passes.
    """
    report = validate(layers=["REAL"])
    assert report.exit_code == 0, report.render()
    assert any(result.status == STATUS_SKIPPED for result in report.results)
