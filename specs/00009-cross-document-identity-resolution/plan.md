...
| FR-046 | Baseline, Metrics, Evidence | `identity/baseline.py`, `compute/metrics.py`, `identity/report.py` | Exact-match honest opponent {AD-012}; same stratum, same estimator, labeled. Import contract keeps it from reading `alias.py` or `compute.pair_score` — it **shares** `normalize.py`, see AD-012 |
