...
| AD-008 | How are the gated requirement and criterion reported? | Fail / pass / excluded | **FR-034 and SC-026 are excluded from the completion denominator** (36 of 37 requirements, 32 of 33 criteria) and reported as `BLOCKED`, never as passing or failing. A fourth state, `WITHDRAWN`, is carried alongside for FR-034's third disposition | They require corpus overlap on manufacturer and part number, which E002 publishes for neither field. Counting them as failing would block `.qc-passed` permanently on another epic's decision; counting them as passing would be a false claim. `BLOCKED` is the only honest third state, and QC tooling must carry it. `WITHDRAWN` is a fourth and not a synonym: if E009's blocking key is amended instead of the corpus, FR-034 is withdrawn rather than met, and rendering that as `BLOCKED` would describe a settled decision as an open dependency. Both states, their printed denominators and their assertability are specified in § Reporting Obligations. |
...
|---|---|
| **(A1) SC-026 renders as `BLOCKED`, and the printed denominator equals the count of `SC-###` IDs defined in `spec.md` — 33 today.** The check MUST derive that number by counting IDs rather than asserting a literal: the counts already moved once, from 26 to 33, during the checklist pass, and a hard-coded literal would have stayed green through exactly that change.** | AD-008 excludes it from the *completion* denominator, but dropping it from the printed fraction would render `32/32` — visually a clean sweep, which is the silent pass the exclusion exists to prevent. The QC report carries an explicit row reading `BLOCKED — excluded from the completion denominator, FR-034 gated on the E002 amendment`, plus a Traceability Gaps line. **Assertable**: the check fails when the printed criterion denominator is not 33, when no row for SC-026 is present, when that row's state is anything other than `BLOCKED` or `WITHDRAWN`, or when the Traceability Gaps line is absent. |
...
| **(A3) FR-034's third disposition — withdrawal — is a distinct printed state** | FR-034 has three exits, not two: satisfied once E002 publishes the fields, **blocked** meanwhile, or **withdrawn** if the resolution is instead to change E009's blocking key (`specs/project-plan.md:344`). A withdrawn requirement that renders as `BLOCKED` misdescribes a settled decision as an open dependency, and one that vanishes from the report misdescribes it as never having existed. On withdrawal, FR-034 and SC-026 render as `WITHDRAWN — E009 blocking key amended at <amendment ref>; requirement withdrawn, not met`, cite the amendment that withdrew them, stay in the printed denominators of 37 and 33, and stay out of the completion denominators. **Assertable**: the check fails when a row is `WITHDRAWN` without a cited amendment reference, and when `BLOCKED` and `WITHDRAWN` are collapsed to one state. |
...
| **(A1, cont.) FR-034 renders as `BLOCKED` on the same terms, and its printed denominator stays 37** | The row above governs the criterion; this one governs the requirement, and both are needed — a report can publish `32/33 + 1 BLOCKED` on criteria and still print a clean requirement sweep by quietly dropping FR-034. **Which denominator it leaves**: the *completion* denominator only — the one `.qc-passed` is computed from, **36** of 37. **Which it stays in**: the printed requirement coverage fraction, denominator **37**, with FR-034 present as a row, never renumbered away and never omitted. **What state must be printed**: literally `BLOCKED`, with the reason `FR-034 gated on the E002 corpus amendment; not satisfiable by this epic`. **What may never be claimed**: `PASS`, `satisfied`, `covered`, `deferred`, or `N/A` — the first three are false, and the last two read as this epic's choice rather than another epic's block. A QC run that cannot render a third state must fail rather than round FR-034 or SC-026 to the nearest one it has. |
