...
| Property | Hypothesis | Every module qualifying as a **deterministic computation module** under the admission rule below — `split.py`, `serialize.py`, `censoring.py`, `posterior.py`, `diagnostics.py`, `shrinkage.py`, `ablation.py`, `likelihood.py`, `compare.py` — **nine**. **Mandatory** under the Testing & Quality Policy, with strict test-first | Pure functions; the sampler is never invoked from this tier. **Where `data-model.md` tiers a rule as Property whose enforcement point is a stored row, the fitted posterior or a job report — `DV-004`, `DV-005`, `DV-007`, `DV-010`, `DV-020` — the property is asserted over an in-memory artifact the job has produced, never over a database read**; the database-backed half of each is the Integration tier's | configured |
...
Strict red-green-refactor binds all **nine** qualifying modules — the seven admitted by clause 3 plus `likelihood.py` and `compare.py`, which the policy's own naming admits — and a process obligation no artifact evidences is indistinguishable from one nobody followed. Following E004's and E005's precedent, `tasks.md` MUST carry an explicit **mandatory red-green pairs** list naming, for each of the nine modules, the property-test task ID that precedes its implementation task ID, and each test task MUST be observed failing before its implementation task begins. The branch history carries a `test:` commit preceding the `feat:` commit for each pair. **E005 closed at six of seven** because two tasks landed in one commit; the pairs list exists so that is a checkable miss rather than an invisible one.
...
| SC-013 | DV-002, DV-030; `ck_held_out_prediction__duration_semantic` and `…__anchor_convention` for the recorded labels; DV-023 for the anchor | NC-5 |
| SC-014 | DV-013 | NC-1 |
...
Success criteria also map through `data-model.md` § Validation Rules (`DV-001`–`DV-039`) and § Negative Controls above, which remain normative on each rule's enforcement point and tier.
...
~ src/model/tests/schema/test_migration_chain.py  # AD-007 (f): the second DECLARED_BLOCKS table
...
**Carried forward, disclosed rather than closed**: the eighteen gaps `G-1`–`G-18` in `data-model.md`, and the seven propagation obligations `P-1`–`P-7` above — each of which is a statement in a document this branch may not edit.
