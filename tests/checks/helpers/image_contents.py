...
def expected_distributions(lock_path: Path = API_LOCK, root: str = "api") -> set[str]:
    """The runtime closure of ``root``, walked from the lockfile.
...
def modeling_module_names() -> set[str]:
    ...
    return {name.replace("-", "_") for name in declared - first_party}
