...
| **FixtureLine** *(record)* | `project_id: text` `^PRJ-[0-9]{3}$`; `vendor_id: text` `^VND-[0-9]{3}$`; `po_number: text` `^PO-[0-9]{3}-[0-9]{4}$`; `line_number: int` 1–3; `material_category: text` ∈ 20 map keys; `description: text` non-blank; `manufacturer: text` non-blank; `part_number: text` `^[A-Z]{3}-[0-9]{5}$` — E002's published `PART_NUMBER_PATTERN`, corrected 2026-07-27: this row still carried a six-and-four-digit form invented before the catalog discharge, contradicting § Line record and the pattern E002 actually publishes; `quantity: decimal-string` >0, **scale exactly 1** (`^(0\|[1-9][0-9]{0,2})\.[0-9]$`); `unit_of_measure: text` ∈ 5 values; `order_date: date`; `need_by_date: date` `CHECK(>= order_date)`; `criticality: int` 1–5; `events: FixtureEvent[]` ≥1 | belongs_to: `DatasetFixture`; has_many: `FixtureEvent`; loads_as: `purchase_order_line`; identity from **Roster**; category from **CategoryMap** | Line state = last event's `to_state`; see **State Machine Conformance** |
...
Target ≥ **60%** of lines catalog-overlapping (FR-034, SC-026); realized share recorded. The complement draws both fields from the catalog but from an entry that does **not** make the line's category, so clause 5 fails and the share is measured against a population that can fall below the floor. It does **not** leave them `NULL`: an earlier draft of this paragraph said it did, which contradicted DV-004's "all six present and non-blank" and the delivered `NOT NULL` on both columns — a complement of nulls would have been refused at load, after the artifact was committed. Corrected 2026-07-27. **This share is measured and reported separately from FR-032's**, and the two are not required to select the same lines.
...
| DV-028 | Catalog-overlap share ≥ 60% under clauses 5–6, measured over all lines. Every drawn `manufacturer` is a catalog `canonical_name` — never an alias — whose `categories` contains the line's `material_category`; every drawn `part_number` carries the `part_number_prefix` of that same entry and matches E002's `PART_NUMBER_PATTERN`. The complement draws a **category-mismatched** catalog entry, so clause 5 fails while both columns stay present and non-blank as DV-004 and the delivered `NOT NULL` require. The share can therefore fall below the threshold rather than being satisfied by construction. Realized share recorded in the ground-truth record |
...
| G-4 | The realized purchase-order size distribution does not match the declared cycle's composition — ~79% one-line, ~15% two-line, ~4% three-line against a cycle that is 60 / 20 / 10 | `min(size, remaining)` clips at `(project, vendor)` group boundaries, and 199 lines across 60 groups leaves ~3.3 lines per group, so a `3` lands on a group with fewer lines left more often than not | **Disclosed, not fixed.** `data-model.md` § Allocation declares a *cycle* and requires only that every order's lines share one project and one vendor and that `line_number >= 2` is exercised; it states no target distribution. Shaping the algorithm to hit 60 / 20 / 10 would be asserting a number no artifact derives — the unattributable-number pattern this epic has corrected twice already. What **is** asserted is the weaker claim the design does make: every declared size occurs at least once, and no size outside the cycle does (`test_every_declared_order_size_actually_occurs`). That check is what catches the per-group cycle reset, under which three-line orders occur exactly once in the whole dataset and the pattern reads as followed when it is not. If a downstream consumer needs the distribution rather than the sizes, that is a requirement to add rather than a bug to fix here |
| G-8 | A loaded row cannot be traced to the generation run that produced it *(renumbered from a duplicated `G-4` found by QC; G-5, G-6 and G-7 were already in use, so this row takes the next free identifier rather than displacing one)* | The delivered schema has no column for a dataset content hash or generator revision, and E005 may not add one | Datasheet limitation **L-7** |
| G-7 | FR-032's clauses 1 and 4 cannot fail for any line, so the four-clause overlap predicate has two discriminating clauses on this dataset | Clause 1 asks whether `material_category` is a key of the committed map and DV-004 requires that of every line; clause 4 asks whether `vendor_id` resolves through the roster and FR-001 requires that of every line. The complement therefore fails clauses 2 and 3 only | **Disclosed, not resolved.** FR-032's "fails every clause" is unsatisfiable in combination with DV-004 and FR-001, and weakening either to make it satisfiable would trade a real guarantee for a rhetorical one. The requirement is restated to "every clause that any line can fail", and clauses 1 and 4 are retained as **membership conditions on the whole dataset** — a future dataset drawing a category outside the map or a vendor outside the roster would fail them, and should. Found by QC, which observed that the per-clause controls in `test_overlap_predicate_control.py` cover clauses 2, 3, 5 and 6 and cannot cover 1 or 4 |
