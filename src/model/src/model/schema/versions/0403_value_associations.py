...
One revision for both, on the same grounds as `0402`: they share the
...
What *is* enforced, on `ingestion_run` in `0400`, is that the declared policy
...
revision: str = "0403"
down_revision: str | Sequence[str] | None = "0402"
...
def upgrade() -> None:
    ...
    Every composite foreign key here is `MATCH FULL` for the reason `0402`
