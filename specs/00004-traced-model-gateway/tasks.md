...
- [X] T040 [OBJ3] {TR-035,TR-045} Implement the gateway-owned connection and own-transaction write in src/gateway/src/gateway/record/writer.py → exports: RecordWriter — record/writer.py: own connection, own transaction, and a connect timeout — the driver's default is no timeout, which turns an unreachable database into a hung process and defeats TR-034 entirely. RecordWriter takes a URL and has no connection parameter, so a caller's transaction cannot own the record
- [X] T041 [OBJ3] {TR-011,TR-012,TR-037,TR-043} Populate the closed field list, resolution mode, fixture key, and pricing timestamp in src/gateway/src/gateway/record/writer.py ← T035:compute_cost ← T040:RecordWriter ← T039:elapsed_ms — the closed 22-column list, written out as literal SQL rather than joined from COLUMNS, with a test tying the two together so the readability costs no drift
- [X] T042 [OBJ3] {TR-048} Verify the pinned price-table version resolves before request construction in src/gateway/src/gateway/config.py and src/gateway/src/gateway/orchestrator.py (VR-025) after:T026 — require_resolvable_price_pin over a PinResolver protocol, so the ordering TR-048 fixes is testable without a database — the position of the call is the requirement
- [X] T043 [OBJ3] {TR-058,TR-077} Emit the absent-cost warning and the invocation-completion log line in src/gateway/src/gateway/record/writer.py after:T041 — log_invocation_complete emits TR-077 event 1 at the terminal outcome rather than after the write, so a spooled invocation still produces its line; the absent-cost warning fires for all three reasons, since a warning that skipped one would train a reader to assume it cannot happen
- [X] T044 [OBJ3] {TR-036,TR-041,TR-045} Implement fail-closed spooling to local SQLite in src/gateway/src/gateway/record/spool.py after:T040 → exports: InvocationSpool — record/spool.py: SQLite, WAL + synchronous=FULL, no status column and no attempt counter — each would be an update, and append-only means inserted once and removed once
- [X] T045 [OBJ3] {TR-052,TR-053,TR-054,TR-077} Implement the invocation-triggered exactly-once drain with depth logging in src/gateway/src/gateway/record/reconcile.py ← T044:InvocationSpool — record/reconcile.py: delete strictly after the commit, conflict target the primary key alone, and every failure path retains the row and returns rather than raising — a raise would fail the unrelated invocation whose connection triggered the drain
- [X] T046 [OBJ3] {TR-035,TR-044,TR-055} Add caller-rollback survival and constraint-pairing tests in src/gateway/tests/test_record_writer.py (VR-016, VR-021) after:T041 — 24 tests. Caller-rollback survival uses two real connections against the live database; the eleven constraint-pairing cases assert the constraint *name*, which is what naming every constraint in the migrations bought
- [X] T047 [OBJ3] {TR-041,TR-052,TR-053,TR-054} Add spool, double-drain, and poisoned-row tests in src/gateway/tests/test_spool_reconcile.py (VR-018, VR-019) after:T045 — 18 tests over the spool and drain. Database failures injected through the writer's connect seam — 'refuse this one write then accept the next' is not something a live server does on request
- [X] T048 [OBJ3] {TR-013,TR-070,TR-071,TR-072,TR-073} [COMPLETES TR-070] Add the forward-only transform, classification, and collision check in src/gateway/tests/test_field_naming.py after:T029 — 17 tests. Transform applied forward only, with an injectivity check and a control that plants a real collision; the pin's three recording sites compared including against the live table comment
- [X] T049 [OBJ3] {TR-012,TR-068,TR-079} [COMPLETES TR-012] Add the information-schema read-contract comparison and repaired-rate query in src/gateway/tests/test_read_contract.py after:T041 — 10 tests comparing the declared field list against the migrated information schema in both directions (HINT-009) — a list checked against writer.COLUMNS would agree with itself. Includes the TR-079 repaired-rate query and its denominator rule
- [X] T050 [OBJ3] {TR-055,TR-069,TR-074} Document the read-contract change, pin-bump, and append-only-by-convention procedures in src/gateway/README.md after:T048 — src/gateway/README.md: the read-contract change, pin-bump and price-correction procedures, each stating the mechanical reason it is that way — the pin-bump section leads with the 1.36.0 error as the worked example

---

## Phase 5: OBJ4 - Content-Hash Fixtures and Offline Replay (Priority: P1) 🎯 MVP

- [X] T051 [OBJ4] {TR-021,TR-027,TR-063} Add mode selection with no default and the GATEWAY_ALLOW_PROVIDER_CALLS opt-in to src/gateway/src/gateway/config.py after:T018 — resolve_mode has no default and no fallback; the opt-in is a separate control with a fixed form, so reaching the provider takes two decisions rather than one
- [X] T052 [OBJ4] {TR-023} Add the replay-mode credential-presence guard over the gateway's own process environment in src/gateway/src/gateway/config.py after:T051 — the replay guard checks the credential's absence and never reads its value — refusing to run when the means of cheating is present is what makes the offline claim structural rather than believed
- [X] T053 [P] [OBJ4] {TR-019,TR-020,TR-028,TR-038} [COMPLETES TR-028] Write failing Hypothesis property tests for canonical serialization and digests in src/gateway/tests/test_compute_hashing.py — 21 property tests. Both directions: same request same key, different request different key — the second is the one whose failure looks like everything working
- [X] T054 [OBJ4] {TR-019,TR-020,TR-038} Implement canonical serialization, the closed hashed-field list, and digests in src/gateway/src/gateway/compute/hashing.py after:T053 — the closure runs the other way round from a list — the hashed set IS the request model's declared fields, so adding a parameter is adding a field. Found that the closure rests on extra='forbid', which nothing had asserted of the real model; now it does
- [X] T055 [OBJ4] {TR-022,TR-033} Implement the content-hash store with provenance sidecars and replay-miss-raises in src/gateway/src/gateway/fixtures.py → exports: FixtureStore — content-hash store with a mandatory sidecar. The sidecar is written last, so an interrupted write leaves a response with no provenance, which both has() and load() treat as absent — the other order would label nothing
- [X] T056 [OBJ4] {TR-037,TR-056} [COMPLETES TR-056] Source replay token counts from fixture provenance and count a lookup as one transport attempt in src/gateway/src/gateway/fixtures.py — replay usage comes from the sidecar so a replayed cost is reproducible; latency measures the replay execution, because that is what actually happened
- [X] T057 [P] [OBJ4] {TR-033} Commit the initial fixture store layout with one provenance sidecar per fixture under src/gateway/fixtures/ — one synthetic exemplar under src/gateway/fixtures/, with a full provenance sidecar and a test that re-derives its key — if the hashing changes the store goes cold, and every miss would look like an unrecorded request
- [X] T058 [OBJ4] {TR-067} Install the autouse network guard and the credential-free child environment in src/gateway/tests/conftest.py — autouse network guard patching both connect entry points, loopback permitted because the record is written over a socket. Its public names live in netguard.py, not the conftest — pytest imports a conftest under its own module name, so a test catching the class by path caught a different object and the guard looked broken when it worked
- [X] T059 [OBJ4] {TR-067} Add the outbound-egress observation check establishing SC-008 in tests/checks/test_no_outbound_egress.py after:T058 — shape assertions at the root, behavioural control in the tier that installs the fixture — a root test would have to reproduce the guard and would then test its copy while the installed one did nothing
- [X] T060 [P] [OBJ4] {TR-063} Assert GATEWAY_ALLOW_PROVIDER_CALLS is absent from every CI environment in tests/checks/test_ci_provider_gate_absent.py — scans workflows, committed .env files and compose files; asserts the control's name and permitted value still match the gateway's own constants, read from source rather than imported
- [X] T061 [OBJ4] {TR-021,TR-022} Add replay end-to-end, miss, hashed-field-change, and mode-absent tests in src/gateway/tests/test_fixtures.py after:T055 — 43 tests over modes, misses and provenance. Made the surrounding-whitespace decision explicit rather than leaving it an accident of strip()

---

## Phase 6: OBJ5 - Credential Redaction and Error Normalization (Priority: P1) 🎯 MVP

- [X] T062 [OBJ5] {TR-062,TR-065} Add the single credential key and the bounded configuration-failure message exclusion set to src/gateway/src/gateway/config.py after:T052 — credential_is_present matches one key by exact name — no prefix, no case-folding, no neighbour scan; configuration_failure_message refuses a detail argument for the credential key rather than redacting one, since a parameter that does not exist cannot be forgotten
- [X] T063 [OBJ5] {TR-024,TR-059} Implement fail-closed credential redaction over the five-sink egress inventory in src/gateway/src/gateway/redaction.py → exports: redact — redaction.py: TR-060's two detectors, recursive over the shapes a sink carries including mapping keys. The exact detector runs first so a real key is replaced whole rather than having its tail rewritten and its prefix left behind
- [X] T064 [OBJ5] {TR-061} Read the credential once at construction and hold it off every repr'd or serialized object in src/gateway/src/gateway/provider.py after:T009 — CredentialHandle reads once at construction; __repr__, __str__, __format__ and __reduce__ all closed, and __slots__ leaves no __dict__ for a debugger or serializer to walk
- [X] T065 [OBJ5] {TR-025,TR-064} Normalize provider exceptions to status, error type, and request id with no chained cause in src/gateway/src/gateway/errors.py and src/gateway/src/gateway/provider.py after:T008 — ProviderError carries three scalars and the writer's RecordWriteError carries the constraint name — both extract at the one point the driver exception is in reach and then drop it
- [X] T066 [OBJ5] {TR-026,TR-066} Default content capture off and apply the closed log field list in src/gateway/src/gateway/config.py, src/gateway/src/gateway/redaction.py, and src/gateway/src/gateway/record/writer.py ← T063:redact — content capture off by default with a fixed enabling value; the toggle is scoped to log output alone, asserted against the record's closed field list so it cannot widen it
- [X] T067 [OBJ5] {TR-030,TR-060} Implement the two-detector scan with a seeded positive case per sink in src/gateway/tests/test_fixture_credential_scan.py after:T063 — 12 tests, one seeded positive per sink. The five differ in what scanning means — files as bytes, log output as what a formatter emits, the exception payload as a rendered traceback — so one seeded case would have evidenced one sink and left four asserted by analogy
- [X] T068 [P] [OBJ5] {TR-024,TR-061} Add repr, traceback, and committed-fixture redaction tests in src/gateway/tests/test_redaction.py — 24 tests over reprs, tracebacks and the store, including every formatting path that bypasses __str__
- [X] T069 [P] [OBJ5] {TR-066,TR-076} Add the per-sink not-captured marker test over record, spool, error, logs, and fixture in src/gateway/tests/test_not_captured.py (VR-037) — 15 tests, one stated position per sink. The fixture store is named as the single sink where the not-captured position does not hold in full — that is what a fixture is
- [X] T070 [P] [OBJ5] {TR-062,TR-065} Add mode, credential-key, and message-exclusion tests in src/gateway/tests/test_config_modes.py — the disclosed limit of TR-062 asserted structurally: credential_is_present takes a mapping and nothing else, so it cannot observe an SDK config file or a keychain and does not pretend to
