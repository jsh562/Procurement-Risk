...
## The Three Digests, Plus the Fourth

Four digest-shaped values appear in this model. They are computed over four different things and must not be substituted for one another. All four share the surface form `^sha256:[0-9a-f]{64}$`.

| Field | Layer | Computed over | Computed by | Purpose |
|-------|-------|---------------|-------------|---------|
| `content_hash` | Both | The committed file's **raw bytes**, exactly as on disk — no parsing, no canonicalization | Validator, on every run, recomputed and never trusted | Detects modification of a file after it was manifested (FR-007) |
| `upstream_digest` | REAL only | The **bytes as retrieved** from the source, at the moment of retrieval | Retrieval work, once; thereafter a historical constant | Makes the verbatim-vendoring claim checkable offline (FR-008, FR-008a) |
| `document_model_hash` | SYNTHETIC only | The canonical serialization of the **pre-render document model** (DM-1…DM-6) — never rendered bytes | Generator, and re-derived by re-running it (VR-040) | The reproducibility comparison (FR-021) |
| `roster_hash` | SYNTHETIC only | The E001 roster's canonical re-serialization | `read_roster()`, consumed verbatim | Roster-drift detection (FR-016, FR-020) |

**The distinction that is easiest to lose**: `content_hash` is over bytes; `roster_hash` is over *content re-serialized canonically*, so reformatting the roster file moves nothing while reformatting a corpus PDF moves everything. The two are not the same kind of digest and a reader who assumes they are will draw the wrong conclusion from a match.
...
FR-011 makes the REAL license basis carry the *document identifier with its revision date*, which is per-document. FR-013 forbids more than one license basis per location. Read literally together, those two make every real location illegal, because every document's basis differs. **Resolution**: the no-mixed-licenses rule is expressed over `basis_id` — the governing-basis component — and the per-document components are expected to differ. This is recorded as a resolved ambiguity rather than applied silently.

| Field | Layer | Constraints |
|-------|-------|-------------|
| `basis_id` | Both | `enum{us-gov-17usc105-ufgs, project-generated-no-third-party-rights}` — closed. The **only** component FR-013 compares within a location |
| `statute` | REAL | Non-empty, e.g. `17 U.S.C. §105(a)`. A bare "public domain (federal work)" is not a basis (research) |
...
| `point_of_use_check` | REAL | `enum{NO_COPYRIGHTED_EXCERPT_FOUND}` — a single admissible value. Any other outcome means the candidate was excluded, and its record belongs in the exclusion ledger, not a manifest (FR-005, VR-024). An enum rather than prose because a prose outcome is not checkable |
| `generated_by_this_project` | SYNTHETIC | `const true` |
| `third_party_rights` | SYNTHETIC | `const "NONE"` |
...
| **RetrievalPolicy** | `source_hosts: array<string>` — allow-listed federal hosts, non-empty; `agency_variants: object` — closed variant token → published section suffix (`UNIFIED` → empty string); `target_sections: array<string>` — long-lead Division 26 / 23 sections with their lead-time justification; `anchor_section: string` = `01 33 00` | VR-021, VR-022, VR-023, VR-025 |
...
**Drift story for these six.** They sit outside every `content_hash` and outside the roster hash, so no digest moves when one is edited. The asymmetry E001 recorded for its convention files applies unchanged here:
...
| MS-4 | Written as **bytes** with `Path.write_bytes`, or with `newline="\n"`. Load-bearing on Windows: manifests are compared byte-for-byte by VR-042, and a `core.autocrlf` rewrite or a default text-mode write would fail that comparison for a line-ending reason unrelated to content. |
...
| Runner | Owns | Why |
|--------|------|-----|
| `corpus-validate` console entry (`src/model/src/model/corpus/validate.py`) | VR-001…VR-039, VR-051…VR-060 — manifest↔file integrity, schema conformance, roster-drift, and re-derivation from emitted PDFs | Reads only committed artifacts. FR-031a requires re-derivation to be **independent of what the generator recorded**, so it must not run inside the generator |
| Modeling-boundary test suite (`src/model/tests/`) | VR-040…VR-050 — determinism, boundary contracts, and every property of the document model | These require *re-running the generator*, which is a different failure mode from reading a corpus. A validator that regenerated to validate could not tell a corpus defect from a generator defect |
...
| VR-007 | Every entry's `layer` equals its manifest's declared `layer`. | CorpusManifestEntry | FR-014 |
| VR-008 | Within one manifest, the set of distinct `license_basis.basis_id` has cardinality exactly 1. The mixed-license condition, expressed over the governing-basis component for the reason given under License Basis. | CorpusLocation | FR-013, SC-005 |
| VR-009 | `location` is a single filename — no separator, no `..`, no leading `/` — and the resolved absolute path is a direct child of the location directory and a descendant of `data/`. An entry naming a file outside `data/` fails. | CorpusManifestEntry | FR-018, SC-007 |
| VR-010 | Every entry names an existing regular file. | Document | FR-006 |
| VR-011 | Every file in a corpus location other than `manifest.json` has exactly one entry, and every entry has exactly one file. Both directions fail; `location` values are unique within a manifest. | Document, CorpusManifestEntry | FR-006, SC-004 |
...
| VR-017 | A REAL entry carries exactly the four common fields plus the eight FR-008 fields — no more, no fewer. | CorpusManifestEntry | FR-008 |
...
| VR-022 | `source_location` is an absolute `https://` URL whose host is in `RetrievalPolicy.source_hosts`. | CorpusManifestEntry | FR-008 |
| VR-023 | A REAL `license_basis` carries `basis_id`, `statute`, `document_identifier`, and `point_of_use_check`, all non-empty, and `document_identifier` **equals** the canonical composition of the entry's own section, variant suffix, and revision date. The restatement cannot drift from what it restates. | CorpusManifestEntry | FR-011 |
...
| VR-027 | A SYNTHETIC entry carries exactly the four common fields plus the six FR-009 fields, and carries **none** of `source_location`, `retrieval_response_status`, `retrieved_at`, `issuing_body`, `masterformat_section`, `agency_variant`, `revision_date`, `upstream_digest`. Asserted twice — by the schema's layer conditional and by an explicit prohibited-key check — so removing the conditional does not silently remove the prohibition. A prohibited field present is a failure, not a blank to ignore. | CorpusManifestEntry | FR-009, SC-005 |
| VR-028 | A SYNTHETIC `license_basis` has `basis_id == project-generated-no-third-party-rights`, `generated_by_this_project == true`, `third_party_rights == "NONE"`, and a non-empty `statement`. | CorpusManifestEntry | FR-012 |
...
| VR-035d | `PAGE_SPLIT_FIELD` derived when a field's label is the last text object on page *n* and its value's first text object is on page *n+1*. | Document | FR-031a |
...
| VR-040 | Re-running the generator from the committed seed against the current roster reproduces a `document_model_hash` equal to the recorded value for **every** synthetic document. | Document, CorpusManifestEntry | FR-021, SC-011 |
| VR-041 | Under the renderer version pinned in the modeling boundary's lockfile, re-rendered bytes equal the committed file bytes. A mismatch is reported **together with the observed and pinned renderer versions**, so a pin change reads as a regeneration event rather than a corpus defect. | Document | FR-021a, SC-011 |
| VR-042 | A re-run under an unchanged seed and roster leaves every manifest **byte-identical** to its committed state; zero entries change. Depends on MS-1…MS-6. | CorpusManifest | FR-009a, SC-012 |
| VR-043 | The generator performs no network I/O, asserted by running it under a socket guard that raises on any connect attempt. | Generator | FR-022, SC-014 |
| VR-044 | The generator invokes no language model, asserted by an `import-linter` forbidden contract from `model.corpus` to `model.llm` and to `gateway`, with `allow_indirect_imports = false` — laundering through an intermediate module is the evasion the contract exists to catch. | Generator | FR-022, SC-014 |
...
| VR-050 | Injector unit tests cover all five irregularity classes and pass. The evidence path for `SCAN_DEGRADATION`, which no structural derivation confirms. | Generator | FR-031b, SC-021 |
...
| VR-060 | No `*.pdf` exists anywhere under `data/corpus/` outside a corpus location. Without it, a stray document placed in an intermediate directory would require no entry and sit unmanifested indefinitely. | Document | FR-006, SC-004 |
| VR-061 | Every SYNTHETIC entry's `generation_inputs` names exactly the four committed artifacts the generator reads, and each recorded digest equals the current sha256 of that file's bytes. A mismatch fails naming the drifted input and every document generated from it, not merely the first. This generalizes the roster comparison to the three supporting artifacts whose loosening previously moved no digest and failed nothing. | CorpusManifestEntry | FR-009b, SC-023 |

**Attribute coverage**: every attribute declared in the Entities table is covered by at least one rule above, except the four recorded under Uncovered Requirements below.
...
| **FR-008a** — the equality's force | VR-018 evaluates the equality | The validator cannot distinguish an `upstream_digest` recorded from the retrieved bytes at retrieval from one back-filled out of the committed file. Back-filled, VR-018 is a tautology that always passes. The distinction is held by the retrieval procedure and by FR-008b, not by any committed check |
...
| **FR-005** — point-of-use copyright check | VR-024 requires the recorded outcome to be the only admissible value; VR-026 requires the opposite outcome to appear in the ledger | The outcome is a recorded human judgement. No check re-derives whether a document reproduces a copyrighted standard's text. The enum makes the judgement *stated and closed*, not *verified* |
...
| **Supporting-artifact loosening** | VR-061 now covers the four artifacts the generator reads — the roster, equipment-category map, field-label vocabulary, and generation config — whose digests are recorded per entry and recompared on every validation run | Narrowed, not closed. The two the *validator* reads rather than the generator — `manifest.schema.json` and `retrieval-policy.json` — are still outside any recorded digest, so a loosening edit to either moves nothing and fails nothing. Held by review of the diff |
| **FR-008b, FR-017, FR-034, FR-035** | — | Not data-model concerns. The re-verification job consumes `source_location` and `upstream_digest` and is excluded from the per-push run by design; the three workflow-trigger requirements are `plan.md`'s |
...
|--------|-------------|-------------|
| Roster reformatted or reordered only | Nothing — `roster_hash` is invariant to file layout **by design** (E001 CS-1…CS-6) | None. Not drift |
...
| Upstream publishes a newer revision of a vendored section | Nothing here; FR-008b would report a digest divergence at the URL | **Not a corpus defect.** The vendored copy stays authoritative and its `revision_date` stays truthful. A re-verification divergence means the *source* changed, never that the committed copy did — reading it the other way is the misinterpretation this row exists to prevent |
...
erDiagram
    ...
    CORPUS_MANIFEST_ENTRY {
        ...
        string roster_hash "SYNTHETIC only, from reader"
...
| FR-008 | CorpusManifestEntry REAL set; VR-017, VR-019, VR-020, VR-021, VR-022 |
| FR-008a | VR-018 — **partial**; the tautology exposure is disclosed |
| FR-008b | Not a validation rule here; consumes `source_location` and `upstream_digest`, excluded from the required check by design |
...
| FR-010 | VR-015 |
| FR-011 | License Basis table; VR-023 |
...
| FR-018 | Physical Artifacts; Access Boundary; VR-004, VR-009, VR-055 |
| FR-019 | Consumed Interface table; VR-045 |
| FR-020 | `roster_hash` field; VR-016, VR-029 |
| FR-021 | DM-1…DM-6; VR-040, VR-039 |
| FR-021a | VR-041 |
| FR-021b | VR-013 |
...
| FR-031b | VR-050; VR-036 (necessary condition only) — **partial**, disclosed |
| FR-032 | VR-037, VR-039 |
| FR-032a | VR-052 |
| FR-033 | `layer` and `irregularity_classes` as recorded fields; VR-014, VR-031; filenames explicitly not load-bearing |
| FR-034, FR-035 | Workflow triggers — no data-model surface |
| SC-001 | VR-025 |
| SC-002 | VR-018 |
| SC-003 | VR-026 |
| SC-004 | VR-011, VR-015, VR-056, VR-060 |
| SC-005 | VR-008, VR-027 |
| SC-006 | VR-002; CorpusManifestEntry REAL set (every evaluator-facing field is an entry field) |
| SC-007 | VR-009, VR-055 |
| SC-008 | Runner split table |
| SC-009 | Disclosed as out of the required check |
| SC-010 | VR-005, VR-034, VR-047 |
| SC-011 | VR-040, VR-041 |
| SC-012 | MS-1…MS-6; VR-030, VR-042, VR-058 |
| SC-013 | VR-029 |
