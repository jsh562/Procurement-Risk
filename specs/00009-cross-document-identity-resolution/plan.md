...
data/
  identity/manufacturer-aliases.csv    + committed alias table, seeded from E002's catalogue {AD-004}. **CSV**, and the content digest is computed over a declared canonical serialization — sorted by (canonical_key, alias_class, normalized_alias), LF endings, no BOM — so the digest is stable across editors. The labeled set declares its canonical form already; the alias artifact needs the same or FR-004's version identifier is not reproducible
