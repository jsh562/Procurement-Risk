...
- **SC-043** [US3]: The run manifest records the attribute weight vector beside the threshold constants, and a run whose weights differ from those calibrated against the verified labeled-set hash refuses and publishes nothing — demonstrated by perturbing one weight.
- **SC-042** [US3]: Every run publishes the exact-match baseline's merge precision and recall beside its own, over the same stratum and by the same estimator, with the baseline labeled as strong or weak. A run publishing either figure without its baseline counterpart fails.
