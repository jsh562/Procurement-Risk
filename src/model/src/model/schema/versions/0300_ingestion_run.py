...
E006's first revision, and the first in the `0300`-`0399` block (FR-040).
`down_revision` is `0103`, E004's head at authoring time -- ordering is
`down_revision` and only `down_revision`, so the jump from `0103` to `0300` is a
naming convention doing its job and not a gap in the chain.
...
`ingestion_run_document` (revision `0301`). A run that reloads 3 of 51 documents
...
query over the associations `0302` adds. A stored count is a second answer that
...
#
# FR-040: `revision` doubles as the four-digit filename prefix -- `0300`-`0399`
# is E006's reserved block, declared in `tests/checks/test_migration_ranges.py`
# before this file was authored. The numbers are never compared to decide what
# runs next, so a gap between `0103` and `0300` is the block partition working
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        CREATE TABLE ingestion_run (
            ...
            -- association in `0302` resolves to it.
    ...
    # selected through `ingestion_run_document` (revision `0301`), not by taking
