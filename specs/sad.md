...
**Primary Dependencies**: Next.js 16 App Router, React; FastAPI, Pydantic, psycopg; PyMC, ArviZ, pandas, NumPy; ONNX Runtime (local CPU inference, INT8 and FP32 graphs both loaded — {SAD:ADR-0023}); Anthropic SDK (`claude-opus-5`)<br>
...
**Scale/Scope**: 30–60 source documents, ~5–15k chunks, ~200 purchase-order lines across 5 projects and 12 vendors, ~44 held-out uncensored delivery events (~120 was assumed and E007 falsified it at a 0.25 split); effectively single-user; no multi-tenancy
...
C4Component
    ...
    Container_Boundary(api, "API") {
        ...
        Component(rerank, "Reranker Session", "ONNX Runtime", "Cross-encoder")
    }
...
|-----------|--------|-------------|-------|
| Evaluation reproducibility | Published metrics reproduce within ±0.01 absolute (recall, MRR, precision) and ±2% relative (CRPS skill) from a clean checkout. MRR's bootstrap interval **bounds** reproduce exactly rather than within tolerance, given the recorded resample count, seed, and bit generator | `reproduce` job rebuilds from digest-pinned image and hash-pinned deps, re-runs the harness, diffs against the committed results manifest; the interval bounds are compared for equality, the point estimates within the band | Tolerance published with its cause; bitwise equality is unachievable for MCMC ({SAD:ADR-0009}). A resampled bound whose seed and bit generator are unrecorded is not reproducible evidence, so both enter the pinned surface ({SAD:ADR-0022}) |
...
| Serving/modeling isolation | API image contains zero modeling-stack packages, where that stack is PyMC, ArviZ and pandas — NumPy is admitted, and the local-inference runtime with its tokenizer is not modeling stack at all once the gateway declares it ({SAD:ADR-0023}) | Assertion over the built image's package list, denylist derived from the modeling entry's direct declarations minus first-party sources minus the shared-infrastructure term | Enforces the boundary that keeps the envelope reachable ({SAD:ADR-0003}). The derived protection is now three heavy packages rather than four: the image is *required* to carry the runtime that {SAD:ADR-0006} and {SAD:ADR-0019} both run on, and it pulls NumPy transitively |
| Retrieval quality — recall@5 | ≥ 0.85 on the frozen 50-item set, with a continuity-corrected Wilson 95% interval | Evaluation harness over exact vector search, inverting the score test on 50 hit-or-miss queries; 5-arm ablation published over both retrieval metrics | Wilson is admissible here because the estimator *is* a binomial proportion ({SAD:ADR-0022}). Exact path removes ANN variance from published numbers ({SAD:ADR-0005}) |
| Retrieval quality — MRR | ≥ 0.70 on the frozen 50-item set, with a percentile bootstrap 95% interval over queries — B = 10,000 resamples, seed and bit generator recorded with the figure | Queries resampled with replacement over the same exact-search path, PCG64 seeded by integer sampling integer query indices, bounds at the 2.5th and 97.5th percentiles of the resample distribution | A mean of continuous per-query reciprocal ranks is not a proportion, so Wilson has no trial count to invert and would publish bounds belonging to a different quantity — the method is selected per estimator, not per table ({SAD:ADR-0022}) |
| Forecast calibration | twCRPS skill ≥ 20% over Kaplan-Meier; 80% PI coverage within 73–87% | Held-out evaluation with reliability diagram and PIT histogram | Kaplan-Meier is the honest opponent; quoted lead time reported for context. The coverage band was derived at ~120 uncensored events and the split realizes ~44, so it is unmeasurable at this scale — published unchanged rather than widened, because a band moves only by pre-registration |
| Merge precision | ≥ 0.95 as a **point estimate** on the frozen labeled set's estimation stratum, published with a 95% interval whose method and sidedness are named — **rule of three at zero observed false merges, one-sided exact binomial (Clopper–Pearson) otherwise**, over a denominator of merges made among that stratum's labeled pairs | Identity-resolution evaluation against the frozen labeled set | Precision is primary; recall ≥ 0.80, also a point estimate, published with a Wilson 95% interval, is secondary by design. **Both targets are point estimates, not interval lower bounds** — read as bounds neither is reachable at this sample size (3/40 = 0.075 for precision even at zero errors; a Wilson interval around 0.80 at ~20 true pairs spans ≈0.58–0.92). The two-branch rule for precision and Wilson for recall follow {SAD:ADR-0022}: the interval method is selected per estimator, and naming only the rule of three left the criterion inapplicable after a single false merge |
...
| ADR-0018 | Two Anchor-Distinguished Posterior Populations in Two Tables | accepted | 2026-07-27 | — | [0018-two-anchor-distinguished-posterior-populations-in-two-tables.md](adrs/0018-two-anchor-distinguished-posterior-populations-in-two-tables.md) |
| ADR-0019 | Embedding Runtime Pinned to ONNX Runtime for Corpus and Query Embedding | accepted | 2026-07-27 | — | [0019-embedding-runtime-pinned-to-onnx-runtime.md](adrs/0019-embedding-runtime-pinned-to-onnx-runtime.md) |
...
- **A weighted lexical field with no writer degrades ranking silently, and nothing in the schema can detect it.** The chunk store's full-text column is a generated `tsvector` concatenating four `setweight` arms — heading `A`, part numbers `B`, specification section `C`, body `D`. Every arm is `coalesce`d, which is deliberate and necessary: `tsvector || tsvector` yields NULL if either side is NULL, so an uncoalesced NULL field would empty the *whole* vector and make the chunk unfindable rather than merely badly ranked. The cost of that correct choice is that an unpopulated arm is indistinguishable from a populated one at the storage boundary — it contributes an empty vector, the concatenation succeeds, the row indexes, every constraint passes, and the ranking signal is simply absent. This is not hypothetical: `chunk.part_numbers` was NULL on all 6,391 ingested chunks, so the weight-`B` arm was inert corpus-wide, and the column carries no constraint that could have said so. It matters more than a missing column normally would because **PostgreSQL's `ts_rank` uses no corpus-wide term statistics** — there is no inverse-document-frequency term to carry relevance when field weighting contributes nothing, so on a synthetic layer with little lexical variety the weighting is the lexical arm's only ranking compensation. Mitigation: the producing epic owns population as an acceptance criterion, and an ingestion run publishes the per-layer proportion of chunks whose weighted fields are all empty, so an inert arm is reported at write time rather than inferred from disappointing retrieval numbers much later.
...
- The modeling entry's console entry points are the inventory of model-owned jobs, and it grows by one per epic that adds one: `migrate`, the four `corpus-*` commands, the three `procurement-*` commands, `ingest`, the two `forecast-*` commands, and **`resolve-identity`** — the offline identity-resolution job (E009). One verb per job, `<domain>-<verb>`, no subcommand dispatcher, so a workflow step names the job it runs.
- Build-gating import contracts are the enforcement surface for the boundaries above, and the set is cumulative rather than per-epic: the single-provider-import contract, the computation-boundary contract, the corpus-offline contract, the baseline-independence contract, and **two added by E009** — `model.identity` may not reach `model.llm` or `gateway`, and `model.identity.baseline` may not reach `identity.alias` or `compute.pair_score`. The second is what makes an honest-opponent claim checkable rather than asserted: a baseline that reads the alias table is the system under test at a lower threshold, not an opponent. Indirect detection stays on for both, because laundering a dependency through an intermediate module is the evasion these contracts exist to catch.
- Published-figure interval discipline: a figure is classified before it is published, and the classification is itself published. A proportion **estimated from a sample** carries an interval. A **census** — a figure computed over the whole of its own population, where every element is observed and nothing is sampled — carries its exact denominator and an explicit declaration, from a closed set the publishing artifact declares, of why no interval applies. Attaching an interval to a census asserts a sampling frame it does not have; publishing one bare leaves it indistinguishable from an estimate whose interval was dropped, which is why the declaration is required rather than optional. This is the engineering form of `project-instructions.md` Principle II as amended at **v1.2.10** and does not narrow it — a Core Principle is narrowed only in that document.
...
- Schema-change coordination fixed: each epic reserves a migration-number block at its start and owns a disjoint table set, so parallel epics adding schema within one wave cannot collide. Alembic is the migration tool; the block is a filename prefix over its revision identifiers, and continuous integration asserts a single head ({SAD:ADR-0013}). **The reservation is committed to the default branch, not asserted in a plan** — allocation is verified by scanning the migration directory there, so a block reserved only on a feature branch is invisible to the next allocator and reserves nothing; `project-instructions.md` v1.2.11 carries the general form of this after four number collisions measured it.
...
- Forecast posteriors are stored as **two populations distinguished by anchor, in two tables**, never as one table with a per-row anchor column ({SAD:ADR-0018}, extending {SAD:ADR-0004} rather than superseding it). As-of-anchored open-line posteriors keep the delivered read contract literally true; order-date-anchored predictions for graded held-out lines live in their own table. A reader that cannot tell the two apart mis-scores every row of one of them with nothing detecting it, and the delivered store carries no column that could say which is which.
- Reproducibility of a *sampled* artifact is claimed per-item on published quantiles within an absolute tolerance whose derivation and **basis condition** are published with the number — never as an aggregate, which can agree while every item moves in compensating directions, and never as bitwise draw equality ({SAD:ADR-0009}). Where the realized sampling conditions fall outside the stated basis, the claim degrades to a reported scope limit rather than passing or failing, the same treatment an out-of-pin library version receives.
- Embedding *runtime* fixed to ONNX Runtime for both corpus embedding and request-time query embedding ({SAD:ADR-0019}), completing {SAD:ADR-0012}, which pinned the model and dimension but named no runtime. One inference runtime loads in the serving container, which already carries the INT8 reranker session inside the 400 MB envelope. The cost is that a raw export emits token-level hidden states only — attention-masked mean pooling and L2 normalization become repository code — so parity against the reference encoder is an asserted numeric tolerance rather than an assumption.
- Derived ingested data carries a generation that is active or superseded, held per document rather than per run because a run re-ingests only the documents whose inputs changed ({SAD:ADR-0020}). At most one active generation per document is a partial unique index rather than application discipline, so two live generations are unrepresentable; downstream readers filter through a view rather than each remembering to. Retirement deletes strictly leaf-up, because `ON DELETE RESTRICT` cannot be deferred the way `NO ACTION` can. **Superseded generations are removed at promotion rather than retained** ({SAD:ADR-0021}, superseding {SAD:ADR-0020} on that clause only): chunk ordinals are unique within a *document*, not within a generation, so two retained generations for one document are unstorable. Reverting a bad promotion is therefore a re-run of the previous chunker version, not a flag flip, and two chunker generations cannot be compared inside one database.
...
- Feature workspace numbers were not allocated the way decision-record numbers are, and two workspaces both took `00002` — E003's and `00002-public-corpus-and-manifest` (E002). **Resolved**: E003's was renamed to `00003-core-data-schema`, so numbering is now one-to-one with epic numbers (`00001`→E001, `00002`→E002, `00003`→E003, `00004`→E004), and `project-instructions.md` v1.2.2 made claiming a workspace number at epic start a rule, with v1.2.3 recording the convention. That rule was necessary and insufficient: it said *when* to claim and never *where*, so the claim landed in a feature workspace the allocator's scan does not read. v1.2.11 completes it — a claim is a placeholder committed to the default branch, and a claim held only on a branch is not a claim.
...
- Convergence diagnostics are a **blocking gate**, not a recorded observation: a fit breaching a published threshold writes no artifact and moves no active-run pointer. A badly-converged posterior produces intervals indistinguishable from a good one, so recording without bounding is the silent-pass failure.
- Declared schema constants are fixed **by decision, not by check** where the enforcement column says so. `forecast_run.draw_count` and `.horizon_days` carry per-run values that no constraint binds to the declared 4,000 and 365; an epic wanting its runs pinned to those figures must assert it itself.
- Train/held-out split ownership is **construction in E007, freeze-and-hash in E014**. Construction sits with the epic that reads the lines; the freeze-before-tuning guarantee sits with the harness that would otherwise tune against its own evaluation set.
- **Local inference lives in the shared gateway package, and the serving image admits its runtime** ({SAD:ADR-0023}). The query encoder and the cross-encoder reranker session are placed in `/src/gateway` — the package both Python boundaries already declare — so `/src/model` calls it for corpus embedding and `/src/api` for query embedding and reranking, with neither boundary declaring the other. Placement is a package fact, not a runtime one: the sessions still execute inside whichever process imports them, so the reranker stays resident in the serving container exactly as {SAD:ADR-0006} requires. Two committed guards had to be narrowed to make the placement buildable, and both were doing their job when they failed — the gateway's own resolved-set isolation check and the in-image denylist each derive the modeling stack from what `/src/model` declares, which included `onnxruntime`, `tokenizers` and `numpy`. The shared-infrastructure term is extended to **NumPy alone**, on `psycopg`'s reasoning: the image is *required* to carry the runtime rather than merely permitted to. Naming the runtime and its tokenizer as exclusions would fail the build instead of fixing it — they leave the derived denylist automatically once the gateway declares them, and excluding a name the modeling boundary no longer declares trips the staleness assertion that every excluded name is still declared by `model`. The cost is not softened: the derived protection narrows from four heavy packages to three, so NumPy could henceforth arrive by an unintended route with nothing reporting it, and the reasoning is required to be recorded at the constant rather than the guard silenced. `project-instructions.md` v1.2.9 carries the matching narrowing in both affected clauses, which had to move together.
- **A denormalized field that feeds a ranking expression is owned by the epic that writes the row, and an unpopulated weighted arm is a defect rather than a tuning matter.** The `search_vector` design and its `A`/`B`/`C`/`D` weights are correct and unchanged; what was missing was an owner for the *data*. The gap sat between two epics that both passed: the schema epic's criterion — full-text search operates against the chunk table with field weighting applied — was satisfied the moment the generated column existed, and the ingestion epic wrote the column NULL as a deliberate, documented refusal to store a guessed value. Neither was wrong on its own terms, and no criterion anywhere asked whether the weighted arm actually carried text. Population is now an ingestion acceptance criterion, scoped to chunks from which a designation was actually extracted rather than to the corpus, because the real specification layer is requirement prose containing no part designations and a coverage target over all chunks would be unmeetable in exactly the way that invites a fabricated value. Relevance tuning remains a query-time weight-array change and never a migration — but that property only holds for arms that have data, and a weight array cannot compensate for an empty one.
- Interval method is fixed **per estimator, not per table** ({SAD:ADR-0022}, extending {SAD:ADR-0009} rather than superseding it). The Quality Attributes row for retrieval quality is now two rows, because one row is what let a single "Wilson 95% CIs" phrase cover two estimators of which only one qualified: recall@5 is a hit-or-miss proportion over 50 queries and keeps its continuity-corrected Wilson interval, while MRR averages continuous reciprocal ranks, has no trial count to invert a score test on, and takes a percentile bootstrap over queries at B = 10,000. Splitting the row is the structural half of the fix — a method named beside its own figure cannot silently re-merge with a neighbour's. The bootstrap adds a seed *and an explicitly pinned bit generator* to the reproducible surface: a seed alone does not fix a bit stream across library versions, so an interval could move on a dependency bump and read as drift to the `reproduce` job. Both published thresholds are unchanged, and the release gate still compares point estimates — the interval is published beside the gate rather than being it, so no metric's pass or fail state moves.
