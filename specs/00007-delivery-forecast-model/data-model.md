...
| DV-005 | Open-line draws are conditional remaining durations, demonstrated by comparison: the decile of open lines with the longest elapsed time has a median stored draw no smaller than the decile with the shortest, and no open line's `survival[1]` falls below the published floor derived from the fitted one-day hazard. **Not** an assertion about `S(0)`, which the delivered array does not store. | Job. **Two halves, two levels**: the between-decile comparison and the floor are asserted at the Property tier over the in-memory draw set the job produced; the same comparison over the stored rows is the Integration tier's | Property + Integration |
