...
**Primary Dependencies**: Pydantic and psycopg (base runtime); Anthropic SDK behind a `provider` optional extra; SQLite from the standard library. No migration tooling — {SAD:ADR-0013} places Alembic and the runner in `/src/model`; its database-client clause is superseded by {SAD:ADR-0016}, under which no migration tooling, ORM, or schema asset lives outside `/src/model` while a driver held for a sanctioned purpose is unrestricted
**Storage**: PostgreSQL 16, single instance — `llm_invocation`, `price_table_version`, `price_table_entry`; plus a committed on-disk fixture store and a local SQLite spool that is a transient buffer, not a datastore of record — it holds only unreconciled writes, no consumer queries it, and its steady state is empty, which is v1.2.1's full three-part test ({SAD:ADR-0015})
...
**Audited against `project-instructions.md` v1.2.1 — re-checked 2026-07-26** after the ADR-0013 reconciliation and the v1.2.0/v1.2.1 amendments. Recording the audited version is what makes drift detectable under the v1.2.0 concurrency protocol; the prior audit ran against v1.1.3 and was flagged by exactly that rule.

**Reassessed against v1.2.3 — no re-run required.** v1.2.2 extends the epic-start number-claiming clause to Feature Workspace numbers. It changes nothing this epic depends on, and this epic already satisfies it: the workspace sits at `00004`, claimed before authoring, and does not participate in the `00002` collision between E002 and E003 that motivated the amendment. Recorded rather than glossed, so the next reader need not re-derive it. v1.2.3 corrects v1.2.2's worked example and records the convention it revealed; neither touches anything this epic depends on. **{SAD:ADR-0016} was applied in both directions during implementation**: E003's `tests/checks` enforcement read ADR-0013's superseded clause literally and forbade the gateway a database driver, which would have collapsed ADR-0015's spool. The check was narrowed to schema *authorship* — the migration and ORM stack, plus a new file-level schema-asset assertion — rather than either record being overruled. **{SAD:ADR-0017}** also landed since the last audit and names E004 among its related artifacts: it permits a Specify-phase requirement to declare a Plan-phase artifact normative for a bounded scope. E004 makes no such declaration — `data-model.md` here is descriptive, and `spec.md` is normative throughout — so the record applies to this epic only as context for E003's arrangement, not as an obligation.
...
| Data Provenance | Committed fixtures carry provenance; no proprietary content; v1.2.0 per-layer provenance | PASS with a deferral — per-fixture provenance sidecar, redaction before write. The two controls are distinct: redaction is credential-only (TR-059); content exclusion rests on E002's corpus constraint, restated as a boundary rule in TR-067. **v1.2.0 per-layer rule**: fixtures are model output, not corpus documents, so neither the retrieved-layer nor the generated-layer field set applies verbatim; TR-033's sidecar is the analogue. Whether the fixture store is itself a dataset carrying the generated-layer set is deferred as spec OI-5 to E014/E015 |
| Governance | Project-wide decisions recorded as ADRs, not buried in the plan; decision-record numbers claimed at epic start; amendments serialized on the default branch | PASS with a recorded miss — ADR-0014 and ADR-0015 authored for this epic, plus ADR-0016 settling ADR-0013's database-client clause, which is what sanctions this epic's only base-runtime dependency. **Miss, disclosed**: numbers were not claimed at epic start, so 0011 and 0012 collided with records landed concurrently on the default branch and had to be renumbered to 0014/0015. That is precisely the failure v1.2.0's claim-at-epic-start rule exists to prevent; the next epic claims its block before authoring. Amendments raised here were performed on the default branch, not on this branch |
...
| AD-009 | How strictly may the public-surface modules be forbidden from reaching `gateway.provider`? | One contract over `api`, `models`, `errors` with indirect detection on / one contract with indirect detection off / two contracts, each set to what its half can bear | **Two contracts**: `models` and `errors` with indirect detection on, `api` with direct detection only | Written as one contract with indirect detection on, the rule forbids `api -> orchestrator -> provider` — which is the arrangement OBJ1's own deliverables prescribe, so `invoke()` would have had no route to the provider at all and the contract would have been unsatisfiable rather than strict. Written as one contract with indirect detection off, the type modules lose the laundering protection they can afford and nothing legitimate needs. Splitting keeps each half at its strongest satisfiable setting. Residual: with indirect detection off on `api`, a type obtained by the orchestrator and re-exposed in an `api` signature produces no forbidden edge — that gap is why T013 is a runtime check over annotations and not a fourth contract |
| AD-010 | In what form does a caller supply the schema of TR-005 and TR-006? | Raw JSON Schema mapping / pydantic model class / either | **A pydantic model class** | A raw mapping needs a JSON Schema validator to enforce post-decode, and the repository has none — adding `jsonschema` would be a runtime dependency bought to re-implement enforcement pydantic already performs, against a spec that treats each added gateway dependency as a cost. Both Python boundaries are pydantic already, and the provider client accepts a model class on its native structured-output parameter, so this is the form the whole path already speaks. `model_validate` then enforces the caller's constraints *in full*, including every one the native mode demoted to a description hint. Cost: a caller holding a schema as data must define a model for it. Reversal: accepting a mapping is additive and needs only a validator dependency, so nothing here forecloses it |
| AD-011 | How does the retry loop of T021 learn how much of the deadline is left, given TR-032 bars `gateway.provider` from `gateway.compute`? | Clock read inside `provider.py` / duration arithmetic moved out of `compute` / an injected `RemainingTime` callable | **An injected callable** | The first puts duration arithmetic in the module TR-028 says must not hold it; the second dismantles the computation boundary to satisfy a contract that exists to protect it. The third keeps the clock with the orchestrator, the subtraction in `gateway.compute`, and leaves `provider.py` comparing a number to zero — no arithmetic, no import edge, and the same `Protocol`-taken-as-argument shape `ProviderClient` already established. Side benefit rather than motivation: the transport budget becomes testable with no clock, no credential and no network, which is what TR-027 asks of the automated suite |
...
| IP-002 | E001 Compose `db` service | Revisions in the `0100`–`0199` prefix block applied by E003's runner in `/src/model` ({SAD:ADR-0013}), invoked as the `migrate` console entry point E003 declares (`model.schema.cli:main`, config at `src/model/alembic.ini`), through the modeling entry's own environment rather than as a container job ({SAD:ADR-0011}); new CI job carries the same `db` service and sets `DATABASE_URL`, which E003's harness reads with no default | `data-model.md` |
...
| TR-012 | Record Writer, models | `record/writer.py`, `models.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py` | Field list is the E013 read contract |
| TR-013 | models, migrations | `models.py`, `src/model/src/model/schema/versions/` | OpenTelemetry gen-AI conventions at a pinned version |
| TR-014 | Compute | `compute/pricing.py` | Pure function over counts, model, version, pricing timestamp |
| TR-015 | Data model | `src/model/src/model/schema/versions/0100_price_table_version.py`, `0101_price_table_entry.py`, `0103_seed_price_table.py` | Composite PK `(version, model, effective_from)`, four rate columns, seed data |
| TR-016 | Compute, Record Writer | `compute/pricing.py`, `record/writer.py` | `cost_absent_reason` column |
| TR-017 | E003 runner (consumed) | `src/model/src/model/schema/versions/` | E004 authors revisions only; the runner and Alembic config are E003's per {SAD:ADR-0013} |
| TR-018 | Revisions, prefix check | `src/model/src/model/schema/versions/`, `tests/checks/test_migration_ranges.py` | `0100`–`0199` prefix block plus a single-head assertion, checked not merely claimed |
...
| TR-031 | Record Writer | `record/writer.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py` | `trace_id` NOT NULL; generated when the caller supplies none |
...
| TR-044 | Record Writer, migrations | `record/writer.py`, `models.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py` | Nullability contract carried by column constraints, not writer discipline |
...
| TR-046 | migrations | `src/model/src/model/schema/versions/0101_price_table_entry.py`, `0102_llm_invocation.py` | `ON DELETE RESTRICT ON UPDATE RESTRICT` on both price-version FKs |
...
| TR-049 | Compute, migrations | `compute/pricing.py`, `src/model/src/model/schema/versions/0101_price_table_entry.py`, `0102_llm_invocation.py` | USD, scale 10 / 6, sum-then-quantize-once, exact-decimal round trip, out-of-range recorded absent (HINT-004) |
| TR-050 | E003 runner (verified against) | `src/model/src/model/schema/versions/`, `src/gateway/tests/test_migrations.py` | Postcondition-defined idempotency verified against E003's runner rather than provided by this epic |
| TR-051 | prefix and single-head check | `tests/checks/test_migration_ranges.py` | One directory, two claimed blocks; absent directory reported not-present rather than passing |
...
| TR-055 | Record Writer, migrations | `record/writer.py`, `src/model/src/model/schema/versions/` | No UPDATE/DELETE statements exist; enforcement disclosed as convention plus referential restriction |
...
| TR-068 | Record Writer, migrations, read-contract check | `record/writer.py`, `models.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py`, `src/gateway/tests/test_read_contract.py` | Field list closed at requirement level; information-schema column set compared against the list, failing either way (VR-032) |
| TR-069 | migrations, docs | `src/model/src/model/schema/versions/`, `specs/00004-traced-model-gateway/data-model.md` | Read-contract change procedure: amend requirement, new higher-numbered migration, record against IP-005; E013 agreement for a removal or rename |
| TR-070 | Config, migrations, naming check | `config.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py`, `src/gateway/tests/test_field_naming.py` | Pin is `1.37.0` (corrected from `1.36.0` by T026 — `v1.36.0` defines `gen_ai.system`, not `gen_ai.provider.name`); three recording points must agree — config key, `COMMENT ON TABLE`, requirement. Task verifies the release defines the classified attributes before implementing |
| TR-071 | models, migrations, naming check | `models.py`, `src/model/src/model/schema/versions/0102_llm_invocation.py`, `src/gateway/tests/test_field_naming.py` | Per-field provenance split; no `gen_ai_` prefix on gateway-local columns; `error.type` and W3C Trace Context Level 1 pinned as their own sources |
...
| TR-074 | migrations, Config | `src/model/src/model/schema/versions/`, `config.py` | Pin-bump procedure: rename in a new migration, three recording points updated together, classification refreshed, TR-069 applied |
...
| TR-081 | migrations | `src/model/src/model/schema/versions/0100_price_table_version.py`, `0103_seed_price_table.py` | Snapshot date and published source mandatory on every version; unsourced version neither seeded nor pinnable |
...
+ src/model/src/model/schema/versions/0100_price_table_version.py    E003 owns the directory
+ src/model/src/model/schema/versions/0101_price_table_entry.py      FK target must exist first
+ src/model/src/model/schema/versions/0102_llm_invocation.py         FK target must exist first
+ src/model/src/model/schema/versions/0103_seed_price_table.py       seed version + entries
...
+ tests/checks/test_migration_ranges.py                prefix blocks, no duplicates, single head
...
~ .github/workflows/verify.yml                         gateway coverage wiring, db service, extras sync
  (governance documents are NOT this feature's files — the needs below were raised
   here and discharged on the default branch per the v1.2.0 serialization rule:
   specs/sad.md, specs/project-plan.md, project-instructions.md v1.2.1)
```
...
| ~~The gateway retains a Postgres driver against ADR-0013's database-client clause~~ **No longer a divergence.** | — | {SAD:ADR-0016} supersedes that clause: schema, DDL, and migration-tooling ownership is exclusive to `/src/model`; a driver held for a purpose an accepted record sanctions is not restricted, and {SAD:ADR-0010} sanctions invocation recording. Spec OI-7 closed. Row retained so the reasoning is not lost |
...
| ID | Item | Disposition |
|---|---|---|
| OI-1 | E004 ↔ E014 relationship and the migration arrangement are not carried by the registered project plan | **Need raised here, propagation discharged on the default branch.** `specs/project-plan.md` now carries: ADR-0011 through ADR-0016 added to its decision table, E014's dependency edge on E004 recorded, and E003's and E004's dependency contracts restated to match {SAD:ADR-0013} — E003 owns the Alembic runner in `/src/model`, E004 authors revisions in its reserved block and builds none. IP-006 retains the disclosure wording so the provenance of each claim stays readable |
| OI-2 | `specs/sad.md` still listed `llm_invocation` retention as an open question while its baseline section recorded the answer | **Need raised here, closed on the default branch.** The stale open question removed; the answer stands in the baseline section, with the spool's own growth bound folded into it (ADR-0015) |
...
| PI-1 | `project-instructions.md` Technology Stack read "no second datastore" flatly, which QC reads as authoritative and would have scored the spool against | **Need raised here, amendment discharged on the default branch** per the v1.2.0 serialization rule — a feature branch records the need and does not perform it. Landed as v1.2.1, "no second datastore *of record*" with a stated non-breach test and an ISO-dated changelog entry citing ADR-0015 |
| PI-2 | The Testing & Quality Policy named Ruff, ESLint/Prettier and `import-linter` but no Python type checker, while OBJ1 VC1 and SC-002 require a consumer to **type-check** against the gateway | **Need raised here, amendment discharged on the default branch.** Landed as v1.2.4, naming mypy and scoping it to `/src/gateway` — `api` and `model` are not annotated to a standard strict mypy would accept, and a repository-wide claim that ran over one entry would be the unverifiable assertion the policy section exists to prevent. No new QC category: "linting" already covers static analysis. `.github/workflows/verify.yml` gains the matching `Type check (gateway)` step, also main-owned and committed separately |
| OI-9 | The gateway's continuous-integration sync resolved it **without** the `provider` extra, which would have failed its own provider tests and its type check for the absence of a dependency the entry is entitled to | **Closed in Phase 2.** The sync step now installs the gateway with `--extra provider` and the other entries without extras. The asymmetry is deliberate and commented in the workflow: the environment that must *not* carry the SDK is the one `tests/checks/test_gateway_no_provider_env.py` builds for itself, which is why that check constructs an environment instead of borrowing an entry's |
| OI-10 | `raise ... from None` was taken as satisfying TR-064, but it sets `__suppress_context__` only — `__context__` still holds the original and `exc.__context__` still returns it | **Closed in Phase 2**, and found by T015 rather than by review. `load_client_class` now raises outside the `except` block, which genuinely clears the context, so the property holds against inspection and not only against traceback rendering. **Recurred in Phase 3** — `with_transport_budget` was written with the same `from None` and T025 caught it the same way. Now closed at both sites, and T065's normalization inherits the placement requirement rather than the idiom |
| OI-13 | E003's schema suite went red the moment E004's revisions landed — 11 failures across three files, all encoding "E003 is the sole author of this chain", which {SAD:ADR-0013} has made false | **Closed in Phase 4 by rescoping E003's checks to authorship, on the user's decision between three options.** Each check kept its claim and had its *proxy* corrected: (1) `test_table_ownership.py`'s forbidden-table check read `pg_class` and required E004's tables to be absent — now scans revision sources `0001`–`0099` for the `CREATE`, which is what its own docstring always said it meant ("a migration in this chain that creates one"), plus a new complementary test that a present table is attributable to a revision *outside* that block, so presence is still audited; (2) its TR-083 documented-object reader and `test_constraint_audit.py`'s data-model reader now read every `specs/*/data-model.md` rather than E003's alone — each epic documents its own objects, instead of the duty landing on whichever epic's tests run last; (3) `test_migration_chain.py`'s block check asserted every revision sits in `0001`–`0099` — now asserts membership in *a declared block*, with the partition claim left where TR-051 puts it, at the repository root. Verified in both directions: an E003 revision creating `llm_invocation` still fails, and E003's suite is green at 1072 passed. **The alternative rejected**: documenting E004's 3 tables and 31 constraints in E003's document, which would invert the ownership ADR-0013 exists to fix and duplicate a document that already exists |
| OI-14 | E003's `test_every_single_column_check_sits_on_a_not_null_column` rejected four of E004's checks that are explicitly null-safe | **Closed in Phase 4, and this one was a genuine over-strictness rather than a scoping proxy.** The test flagged any single-column `CHECK` on a nullable column, using column count as a proxy for TR-039's real rule — *a check must not be silently satisfied by a NULL*. E004's four are written `col IS NULL OR <predicate>`, which is definitely `true` on a null rather than null-valued, with nullability itself governed by a separate paired constraint. The exemption added is not a new standard: it reuses `_null_guards`, the same helper and the same criterion this module's own sibling test already applies ("through `coalesce`, through an explicit null test, or through `num_nonnulls`"), so the two cannot drift apart. Without it the only way to pass would have been to make a check reference a second column it does not need. E004's obligation under the same requirement was met rather than avoided: `data-model.md` gains a **Nullable-column checks** table justifying all eight |
| OI-12 | TR-070 pinned semconv `1.36.0` "as the version carrying `gen_ai.provider.name` under that spelling" — a selection reason that is **false of that release** | **Closed in Phase 4 by T026, which is the verification TR-070 mandates rather than an incidental discovery.** The registry published at tag `v1.36.0` defines `gen_ai.system`; `gen_ai.provider.name` first appears at `v1.37.0`, where `gen_ai.system` is marked deprecated and *replaced by* it. OBJ3 VC7 would have failed against the old pin — the pin working as designed. Corrected to `1.37.0` across all five recording sites in one change, per TR-070's "correct this requirement's version together with that classification": TR-070, TR-071's `error.type` source, `data-model.md` §Field Naming Alignment and its traceability row, the plan's coverage map and HINT-008, and `config.py`. Verified in the same pass and recorded in `data-model.md`: all six gen-AI attributes present at 1.37.0; `error.type` present and marked **Stable**, which is what TR-072's Stable class for `error_type` rests on. **`cache_read_input_tokens` stays Gateway-local** — no cached-input-tokens attribute exists at either release, so the move that row anticipated does not happen at this pin |
| OI-11 | TR-005 says the native mode's unsupported constraints become post-decode validators, but nothing said *which* constraints those are | **Closed in Phase 3, empirically rather than by transcription.** The provider SDK ships the transform that decides it, and `provider.native_output_schema` exposes it so `residual_constraints` derives the set by diffing rather than restating it — a hand-kept list would be a second statement of the same fact with nothing comparing the two. Confirmed in a test: `minimum`, `maximum` and `minLength` survive only as prose in the schema's `description`, where the model *might* follow them. Disclosed limit: the transform sits behind a private module path, so `native_output_schema` returns `None` if an upgrade moves it — enforcement is unaffected, since `validate_or_repair` validates against the caller's schema in full, and a test asserts reachability so the degradation is visible rather than silent |
...
- **[HINT-008]** Order and gotcha: read the pinned convention document *before* writing `0102_llm_invocation.py`. TR-070 pinned `1.36.0` and the check found it wanting: that release defines `gen_ai.system`, not `gen_ai.provider.name`. Corrected to `1.37.0`, where `gen_ai.system` is marked deprecated and replaced. **This hint is retained rather than deleted** — it is the hint that worked, and the next pin bump needs the same procedure. Write the naming check to transform attributes *into* column names only (TR-073); an inverse transform looks reasonable and is wrong, because attributes carry underscores inside their own segments.
