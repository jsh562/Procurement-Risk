...
- **TR-030**: System MUST record residual probability mass beyond the survival horizon as an explicit value, such that the array and the residual account for the full distribution.
...
- **TR-047**: System MUST expose shared schema values through the single-row schema-constants table so `/src/api` reads them over the connection, never by importing from `/src/model`.
...
- **TR-057**: System MUST treat a document row's manifest key as identifying one fixed revision of a source document, so a superseding revision is loaded as a new document row under a distinct key and every existing citation continues to resolve to the revision it was extracted from.
...
- **TR-064**: System MUST oblige a reader that does not recognise a stored artifact's schema version to refuse to read the arrays and report no usable forecast, rather than reading array offsets under an assumed layout.
...
- **TR-080**: System MUST expose the run's as-of anchor date so a reader computes the active artifact's age itself, and MUST impose no maximum permitted age on the active forecast run; the staleness threshold and its interface treatment belong to E010.
- **TR-081**: System MUST treat per-field confidence as a self-reported score asserted by the extracting agent at the recorded extraction time, not as a calibrated probability, so no reader may interpret it as a frequency or compare it across fields as one.
- **TR-082**: System MUST identify the agent responsible for a citation at run granularity through the ingestion run that wrote it rather than on the extracted-value row, so the storage boundary carries the extraction time as its only per-row temporal fact.
- **TR-083**: System MUST define every table and column it creates in `data-model.md`, which is normative for reader-facing semantics, and MUST NOT create an object absent from it — the understandability guarantee that `field_vocabulary.label` and `.description` give vocabulary terms, given at document level for tables such as `line_posterior` and `forecast_run` that carry no such column.
- **TR-084**: System MUST make an extracted-value or extraction-failure row append-only once written, enforced by revoking `UPDATE` and `DELETE` on both tables from the application role in a migration rather than left to caller discipline: a correction is made by removing and reloading the affected chunks in the order the restricting foreign keys permit, never by editing a stored citation, page, confidence, or outcome in place.
...
- **TR-085**: System MUST retain every extracted-value and extraction-failure row for the life of the database, since retention policy is out of scope and the full dataset is regenerable from the repository and its jobs.
...
The analysis also found that the checklist pass appended 33 requirements without integrating them, leaving roughly 13 duplicate clusters and 24 obligations with no acceptance coverage. Those are recorded as A-007, A-008, A-011, and A-012 and deliberately left unapplied — collapsing them would reverse an explicit decision to keep all 33.
