...
The same physical material appears three times in a project's paper trail under three different names — a specification section calls for a manufacturer's switchgear by catalog family, a submittal names the manufacturer's full legal entity with a model number, and a purchase-order line abbreviates both and adds a unit convention neither of the others used. A coordinator asking "has the switchgear I specified actually been ordered, and when does it land" cannot answer it without joining those records by hand. Only part of that join is recoverable from the delivered corpus: submittal and purchase-order records carry the manufacturer and part number needed to match, while the specification layer is real, verbatim-vendored federal text carrying neither, so it is associated at equipment-category granularity rather than merged. Nothing downstream — the risk-ranked worklist, the evaluation harness, the line-detail trace — can attribute a forecast to a specification requirement while that join is missing. The cost of getting it wrong is worse than the cost of not doing it: a wrong merge silently attributes one material's delivery history to another, and no surface in the system displays anything that would reveal it.
...
### User Story 1 - Link the same material between submittal and purchase order, and associate it to its specification (Priority: P1)

A coordinator looking at a purchase-order line can see which submittal proposed that material, because the two records have been resolved to one identity despite naming it differently — and can see which specification section it answers, associated at equipment-category granularity rather than merged.
...
**Why two members and not three**: The specification layer is real, verbatim-vendored federal text. E002 prints manufacturer and part number into **synthetic** documents only, so a specification section carries no manufacturer string and no part number to block or score on. A three-member merge is not achievable from the delivered corpus at any implementation quality, and the association is what the data can actually support. Published as a limitation rather than presented as a shortfall.

**Independent Test**: Run resolution over a project's records and demonstrate that a known submittal/purchase-order pair appears as one resolved entity with both members traceable to their source records, carrying an equipment-category association to its specification section.
...
7. **Given** a resolved entity whose members answer a known specification section, **When** the entity is inspected, **Then** it carries exactly one equipment-category association to that section, and an attempt to admit a second is rejected naming the offending record.
...
- **FR-041**: A ResolvedEntity MUST carry **at most one equipment-category association to a specification section**, and MUST reject an attempt to admit a second, naming the offending record. Purchase-order lines are unbounded within a cluster, because partial shipments and change orders legitimately produce several against one specified material. *(Re-pointed at stress-test STF-006: the bound previously capped specification-section cluster members, which SC-001 established are never members, making it unviolatable.)*
...
- **SC-041** [US1]: A ResolvedEntity carries at most one equipment-category association to a specification section, and an attempt to admit a second is rejected naming the offending record.
...
- Q: STF-002 — SC-001 demoted the specification member but US1 still required a three-member cluster. How should US1 read? -> A: Rewritten to the submittal-to-purchase-order link, with the specification associated at equipment-category granularity and a requirement mandating that association (FR-041). The three-way merge is not achievable from the delivered corpus at any implementation quality, and the story now says so.
