...
| Runner | Owns | Why |
|--------|------|-----|
| `corpus-validate` console entry (`src/model/src/model/corpus/validate.py`) | VR-001…VR-033, VR-035…VR-039, VR-051…VR-068 — manifest↔file integrity, schema conformance, roster and generation-input drift, corpus-root closure, re-derivation from emitted PDFs, path containment and the link prohibition, and the empty-population guard. **59 registered rules**, VR-035 registered as its four sub-rules VR-035a…VR-035d, and **VR-034 is not among them** — see the row below | Reads only committed artifacts. FR-031a requires re-derivation to be **independent of what the generator recorded**, so it must not run inside the generator |
