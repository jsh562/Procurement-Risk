...
- [X] T072 {TR-053,TR-057,TR-062,TR-064,TR-077,TR-078,TR-079,TR-080,TR-081,TR-082,TR-085} Reclassify the eleven non-obligations phrased as MUST (A-011) — specs/00002-core-data-schema/spec.md
...
- [X] T073 {TR-063} Record A-007 as WILL-NOT-MERGE with the evidence — specs/00002-core-data-schema/spec.md
...
- [X] T074 {TR-056,TR-065,TR-076,TR-083} Record A-012 as accepted, with its mitigation and its carry-forward — specs/00002-core-data-schema/spec.md
  > A-012's text was **not recoverable from the working tree** — the superseding analysis pass renumbered to a B-series and overwrote `analysis-report.md` in place while three other files still cited the A-IDs, leaving four IDs with three descriptions between them. Recovered from `git show 7138026:specs/00002-core-data-schema/analysis-report.md`: four requirements make `data-model.md`, a Plan-phase artifact, normative over Specify-phase requirements, so a Plan re-run can invalidate a spec requirement. Accepted rather than corrected — the alternative lifts thirteen tables of DDL into `spec.md`, duplicating the normative source instead of removing the inversion. Mitigation already in practice: all four in-flight `data-model.md` amendments are labelled corrections carrying PostgreSQL 16 evidence and add no constraint name or object. **An ADR is required to bind this beyond E003 and must be authored on `main`** — carried forward with AR-1 and AR-2, not applied from this branch.
