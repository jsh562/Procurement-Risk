...
| FR-040 | report, paths | `forecast/report.py`, `forecast/paths.py` | Three emitted files, enumerated rather than categorised, with the run report a **closed schema** of typed sections. SC-026's absence check becomes a structural predicate over declared fields rather than a term search — which is what removes the carve-out SC-025 and L-3 would otherwise force, since they *require* the report to discuss the registered coverage band |
