...
def _build_pristine_corpus(root: Path, committed_root: Path) -> Path:
    ...
    # over **this tree's** copy of each file, because VR-061 resolves those keys
    # against the corpus root under validation rather than against the
    # repository's.
    generation_inputs = {
        relative: sha256_of_bytes((root / "synthetic" / Path(relative).name).read_bytes())
        for relative in GENERATION_INPUT_PATHS
    }
    # VR-030's three, taken from the copied configuration rather than restated.
    config = json.loads((root / "synthetic" / "generation-config.json").read_text(encoding="utf-8"))
