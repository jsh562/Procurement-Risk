...
| `generation_inputs.*` | SYNTHETIC only | Each remaining generation input's **raw bytes** — equipment-category map, field-label vocabulary, generation config, manufacturer catalogue | Generator | VR-061, from the files | Supporting-artifact drift detection (FR-009b, SC-023) |
...
| **ManufacturerCatalog** | Added by FR-037. `manufacturers: array<object>` — each with a canonical name, at least one alias spelling, a part-number prefix, and the equipment categories it makes; `CHECK(every part number a manufacturer issues begins with that manufacturer's prefix)`; `CHECK(no canonical name or alias matches the roster's real-firm exclusion list after the roster's normalization)`; `CHECK(no printed spelling claimed by two manufacturers)`; a catalogue failing any check fails at load rather than yielding a partial catalogue (FR-037b) | VR-070, VR-071, VR-072, VR-073 |

**Drift story for these seven.** None is a corpus document, so none is covered by a `content_hash`. Four of the seven — `GenerationConfig`, `EquipmentCategoryMap`, `FieldLabelVocabulary`, `ManufacturerCatalog` — are read by the generator and are therefore digested into every entry built from them via `generation_inputs` (FR-009b, VR-061), which moves the loosening exposure below off them. The other three — `ManifestSchema`, `RetrievalPolicy`, `ExclusionLedger` — are read only by the validator, sit outside every recorded digest, and are where the exposure remains. These seven plus the synthetic layer's datasheet are the eight artifacts FR-018a closes the corpus root over (VR-065). The asymmetry E001 recorded for its convention files applies unchanged to those three:
...
| Runner | Owns | Why |
|--------|------|-----|
| `corpus-validate` console entry (`src/model/src/model/corpus/validate.py`) | VR-001…VR-033, VR-035…VR-039, VR-051…VR-073 — manifest↔file integrity, schema conformance, roster and generation-input drift, corpus-root closure, re-derivation from emitted PDFs, path containment and the link prohibition, and the empty-population guard. **64 registered rules**, VR-035 registered as its four sub-rules VR-035a…VR-035d, and **VR-034 is not among them** — see the row below | Reads only committed artifacts. FR-031a requires re-derivation to be **independent of what the generator recorded**, so it must not run inside the generator |
| Modeling-boundary test suite (`src/model/tests/`) | VR-034 and VR-040…VR-050 — the synthetic layer's document floor, determinism, boundary contracts, and every property of the document model. `plan.md` §Testing Strategy assigns each to a tier: VR-050 to the unit tier, VR-034, VR-040a/VR-040b, VR-041, VR-042, VR-043, VR-046, VR-047, VR-048, and VR-049 to the integration tier, with VR-044 discharged by the workflow's `lint-imports` step plus its committed negative fixture and VR-045 by the existing repository-root scan | These require *re-running the generator*, which is a different failure mode from reading a corpus. A validator that regenerated to validate could not tell a corpus defect from a generator defect. **VR-034 sits here for that reason and not by oversight**: its failing direction is a layer the generator emits below the floor, which only a generation run can produce — `src/model/tests/test_corpus_generate.py` asserts the floor over the reference layer and rejects a four-per-project layer built by the real pipeline. No coverage is lost by its absence from the validator; the runner counts differ by one |
...
| VR-034 | The synthetic layer holds ≥ 25 documents across the five locations. Assigned to the **modeling-boundary suite**, not to `corpus-validate` (see the runner split above): the floor's failing direction is a layer the generator emits below it, which only a generation run produces. | CorpusLocation | SC-010 |
...
| VR-061 | Every SYNTHETIC entry's `generation_inputs` names exactly the four committed supporting artifacts the generator reads over raw bytes — `generation-config.json`, `equipment-category-map.json`, `field-label-vocabulary.json`, `manufacturer-catalog.json` — and each recorded digest equals the current sha256 of that file's **raw bytes**. The catalogue joined this set with FR-037; the key set is compared against `GENERATION_INPUT_PATHS` rather than a restated literal, so the enumeration here and the generator's tuple cannot drift apart. **These keys are path-valued and externally controlled, so containment applies to them too** (CWE-73): each key is compared as a literal string against that closed four-value set of repository-relative paths *before* any filesystem access, and the path it denotes is resolved and asserted under `data/corpus/` by VR-009's order of operations and VR-067's link prohibition. A key outside the closed set fails on set equality and is never opened, so no traversal sequence in a manifest-supplied key reaches a resolution step. The supporting-artifact paths the validator itself reads are validator-owned literals enumerated by VR-065, not manifest-supplied, and are subject to the same resolution and link rules. The roster is the fifth generation input and is deliberately not a key here; its digest is `roster_hash` and its comparison is VR-029's, because the reader's value is over canonical content rather than bytes. A mismatch fails naming the drifted input and every document generated from it, not merely the first; attribution is by recorded field — a document is generated from an input exactly when its own entry records that input — never by assuming every synthetic document shares every input. | CorpusManifestEntry | FR-009b, SC-023 |
...
| VR-065 | Under `data/corpus/` and outside every corpus location, the only files present are the eight committed supporting artifacts — `manifest.schema.json`, `retrieval-policy.json`, `exclusions.json`, `generation-config.json`, `equipment-category-map.json`, `field-label-vocabulary.json`, `manufacturer-catalog.json`, `datasheet.md`. Any other file fails, whatever its extension. VR-060 closes the same hole for `*.pdf` specifically; this closes it for every other format, so a stray non-PDF cannot sit outside a location requiring no entry. | Document, CorpusLocation | FR-018a, FR-006 |
...
| VR-069 | **Both halves of the product identity are printed.** Every SYNTHETIC document prints at least one `manufacturer` and one `part_number` field, and prints them in equal number — one pair per material item. Read from the emitted document, never from what the generator recorded | Validator | FR-037, SC-029 |
| VR-070 | **Every printed manufacturer is reconcilable.** Each non-blank printed manufacturer value resolves to exactly one catalogue entry under the roster's normalization, so an alias spelling is recoverable and an unknown name is a failure. Blanks are admitted — `MISSING_OR_BLANK_FIELD` produces them deliberately and VR-069 has already established the field was printed | Validator | FR-037a, SC-029 |
| VR-071 | **The blocking key survives an un-normalized alias, and it recovers the right maker.** Each non-blank printed part number matches `<prefix>-<five digits>` and its prefix names a catalogue manufacturer, so blocking on the prefix recovers *a* maker even when the printed name has not been normalized yet — and the prefix must name the manufacturer printed **for the same item**, the two fields being paired positionally in document order because the generator emits them adjacently per item. Without the second half a document naming one maker beside another's part number passes every rule while blocking on the prefix recovers a maker the document never named, which is a silent wrong merge rather than a visible missing one | Validator | FR-037a, SC-029 |
| VR-072 | **Every equipment category has a maker.** Each category in the committed equipment map is backed by at least one catalogue manufacturer. Without this a category could carry a specification section and no manufacturer, and the defect would surface as a generator failure on a corpus that had validated clean | Validator | FR-037, SC-029 |
| VR-073 | **Synthetic manufacturer names, enforced against the roster's list — and alias spellings actually exercised.** No catalogue canonical name or alias matches the roster's real-firm exclusion list after the roster's normalization, and no printed spelling is claimed by two manufacturers. Reuses E001's list rather than maintaining a second one, because two lists eventually disagree about what counts as a real firm. Additionally, when the SYNTHETIC layer is in scope and non-empty, **at least one alias spelling appears on a committed document**: carrying aliases in the catalogue is not the same as exercising them, and a generator that drew the canonical name every time would leave the catalogue untouched and SC-029 unmet | Validator | FR-037b, SC-029 |
...
| **FR-031a** — independence of the re-derivation | VR-035 and VR-035a…VR-035d read the emitted PDF and never the generator's state, and are additionally exercised against hand-authored fixtures whose expected class sets are written in the test and whose vocabulary is a fixture rather than the committed file | The injector and the deriver both read `field-label-vocabulary.json`. A misreading common to both — an alternate label recorded under the wrong canonical key, say — makes the corpus comparison agree for the wrong reason, and no committed check observes that. The fixture tests stand in for a second, independently authored vocabulary; nothing re-derives the vocabulary itself, and it is one of the four inputs `generation_inputs` does cover, so a *change* to it is detectable even though a *mistake* in it is not |
| **FR-032 / FR-021** — the retained text layer's oracle (VR-039) | VR-039 compares each page's extracted text, NFC-normalized and whitespace-collapsed, against the document model's per-page text, and fails naming the document, the page, and both strings. The extracted side is read from the emitted PDF's bytes. It is what catches a degraded page whose invisible layer lost lines — a defect VR-037's anchor check admits, since such a page still extracts *some* text | The model side of that comparison is **composed, not recorded**. No per-page text is stored in any manifest, so the rule calls `generate.compose_layer`, which plans, injects and composes from the committed generation config, roster and vocabulary — writing nothing, rendering nothing, touching no path under the corpus root. The consequence is that a generator change moves this rule's oracle as well as the corpus, so the two can drift together and this comparison keeps agreeing. Every *other* rule in this runner, VR-035a…VR-035d above all, derives from the emitted bytes alone; this is the single exception. What detects that drift is on the other side of the runner split and not in `corpus-validate`: VR-040a compares a regenerated hash against **the value recorded in the committed manifest**, which a generator change does move away from. Disclosed here because that is where this epic's partial-coverage claims are read; `validate.py`'s module and rule docstrings carry the same statement at the code |
...
| **Supporting-artifact loosening** | Five generation inputs are covered: the roster by `roster_hash` and VR-029, and the equipment-category map, field-label vocabulary, generation config, and manufacturer catalogue by `generation_inputs` and VR-061 — recorded per entry and recompared on every validation run | Narrowed, not closed. **Three** of the eight supporting artifacts FR-018a enumerates sit outside every recorded digest, not two: `manifest.schema.json`, `retrieval-policy.json`, and `exclusions.json`, all read by the *validator* rather than the generator. A loosening edit to any of the three moves nothing and fails nothing. Held by the **pull-request review gate of FR-036**, not by an unassigned expectation that someone reads the diff: an edit to any of the three requires explicit reviewer sign-off on its loosening direction, with the **epic owner accountable**. That is an assigned control, not a mechanical one — no committed check observes the sign-off, which is why this row stays in this table rather than moving into the covered set |
| **FR-009b** — enumeration currentness | VR-061 checks that the recorded mapping names exactly the four enumerated raw-byte generation inputs and that each digest is current; VR-029 does the same for the roster, the fifth | The enumeration itself is closed by maintenance, not derivation. Nothing reads the generator's source to confirm the list still equals what it opens, so a generator that gains a sixth committed input while the enumeration stays at five would record a complete-looking mapping over an incomplete set. That is not hypothetical: FR-037 added the manufacturer catalogue as a generation input and the enumeration was extended in code before the prose followed, which is the failure this row describes. FR-009b states the obligation to extend the list in the same change, and **FR-036 assigns it**: adding a generator input to the enumeration requires explicit reviewer sign-off on the loosening direction, with the epic owner accountable. Named and owned, still not derived — nothing reads the generator's source to confirm the list, so this remains an assigned control rather than a check |
...
| A generation input other than the roster edited — equipment-category map, field-label vocabulary, generation config, or manufacturer catalogue | VR-061 | Fails naming the drifted input and every document whose own entry records it. Attribution is per entry, so a document built before the input existed is not implicated by an assumption that every synthetic document shares every input |
...
erDiagram
    ...
    CORPUS_MANIFEST_ENTRY {
        ...
        object generation_inputs "SYNTHETIC only, four raw-byte digests"
...
| FR-037 | ManufacturerCatalog (Supporting Artifacts); `generation_inputs` — the catalogue is the fourth raw-byte key (FR-009b, VR-061) and the eighth artifact FR-018a closes the root over (VR-065); VR-069, VR-072 |
| FR-037a | ManufacturerCatalog; VR-070, VR-071, VR-073 (alias-presence half) |
| FR-037b | ManufacturerCatalog; VR-073 |
| SC-001 | VR-025 |
| SC-002 | VR-018 |
| SC-003 | VR-026 |
...
| SC-023 | VR-029 (roster half), VR-061 (the four raw-byte generation inputs) |
...
| SC-029 | VR-069, VR-070, VR-071, VR-072, VR-073 |
