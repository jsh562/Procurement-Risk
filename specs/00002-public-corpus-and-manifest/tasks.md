...
**Notes**: 40 of the 72 rules are discharged here, grouped by what they read rather than one task per rule. Rule totals: 3 + 10 + 11 + 3 + 2 + 4 + 4 + 3 = 40. The remaining 32 are Phase 5 (19) and Phase 6 (13). Two orderings are load-bearing: T028's containment and link tests use non-following stats throughout — a symlink to a regular file passes a link-following `is_file()` test exactly, which is why VR-067 exists separately — and T029 resolves the real path **before** comparing against the entry's own location directory, never after. T030's VR-018 is an internal-consistency check, not a provenance proof; its force is conditional on FR-008c and the residual is published. The validator never re-runs the generator: anything requiring regeneration lives in the test suite (Phases 5–6), because a validator that regenerated to validate could not tell a corpus defect from a generator defect. **Counts recorded before FR-037 (2026-07-26).** T064 registered VR-069…VR-073, so the rule total is now 77 and the phase arithmetic above describes the plan as it stood when Phase 4 was written, not the shipped rule set.
...
- [X] T064 {FR-037} Print a manufacturer and a part number on every synthetic material item: commit data/corpus/synthetic/manufacturer-catalog.json as a fourth generation input, add src/model/src/model/corpus/manufacturers.py, extend the field vocabulary and the generator, register VR-069…VR-073 with a failing-direction case naming each (SC-025, SC-029), and regenerate the layer — src/model/src/model/corpus/manufacturers.py after:T063
...
- [X] T065 [BUG:ERROR] {FR-037} [test-failure] The reproducibility fixture cannot import the generator in its second checkout — src/model/tests/test_corpus_generate.py:83
  > ManufacturerCatalogError: cannot read the manufacturer catalogue .../second-checkout/data/corpus/synthetic/manufacturer-catalog.json
  > Errors 4 tests at setup: test_vr_040a_every_document_model_hash_reproduces_the_recorded_value, test_vr_041_rerendered_bytes_equal_the_reference_bytes, test_vr_042_a_rerun_leaves_every_manifest_byte_identical, test_vr_042_manifests_carry_no_line_ending_written_by_the_platform
  > Fix hint: GENERATOR_INPUTS is a second, restated literal the production tuple does not reach. Derive it from model.corpus.manifest.GENERATION_INPUT_PATHS rather than adding a fifth string, so manifest.py's claim that "adding a member here is the whole change" becomes true instead of aspirational.
- [X] T066 [BUG:ERROR] {SC-029} [requirement-gap] VR-070 and VR-071 report a population of 50 and check 34 — src/model/src/model/corpus/validate.py:1845
  > _printed_field_values only harvests a value sitting on the same text line as its label; the renderer emits 16 of 50 across two lines. Measured: 34 same-line, 16 empty-tail, 0 genuinely blank.
  > Fix hint: read the following line when the label line carries no value. The docstrings' justification for admitting blanks does not hold — PRJ-004-T0003-R0.pdf declares irregularity_classes [] and still reads blank.
- [X] T067 [BUG:ERROR] {SC-029} [requirement-gap] Two SC-029 clauses are asserted by no rule — specs/00002-public-corpus-and-manifest/spec.md:292
  > (a) "every part number begins with the prefix of the manufacturer that item names" — VR-071 checks only that a prefix names some catalogue entry, never pairing a number with the manufacturer printed for the same item. (b) "at least one alias appears in the committed corpus" — no rule asserts it.
  > Fix hint: both hold in fact over the committed layer (0 mismatches, 22 distinct spellings printed). Either implement the pairing and the alias-presence check, or narrow SC-029 so it stops delegating to rules that do not judge it.
- [X] T068 [BUG:WARNING] {FR-018a} [requirement-gap] Five sites still state the pre-amendment counts — specs/00002-public-corpus-and-manifest/spec.md:197
  > FR-018a enumerates seven supporting artifacts and omits the manufacturer catalogue, while VR-065's own failure message says eight. Also data-model.md:18 ("seven"), plan.md:200 ("seven"), manifest.schema.json:203 ("remaining three generation inputs"), validate.py:2684 ("the other three generation inputs").
- [X] T069 [BUG:WARNING] {SC-026} [coverage-gap] manufacturers.py is 72%, the lowest file in its package — src/model/src/model/corpus/manufacturers.py
  > Almost every _fail(...) branch in load_catalog is uncovered. The aggregate gate passes at 92% by averaging, which is the case verify.yml's second gate says it exists to prevent.
- [X] T070 [BUG:WARNING] {SC-010} [test-failure] Port search can overflow rather than fail cleanly — tests/checks/test_ports.py:117
  > OverflowError: bind(): port must be 0-65535. a_free_port() can return a port near 65535 and range(start, start + SEARCH_SPAN + 2) runs past the limit; the loop catches only OSError. Failed once, passed on two re-runs — genuinely flaky, pre-existing, not caused by the FR-037 amendment.
- [X] T071 [BUG:ERROR] [RECURRING] {FR-009b} [requirement-gap] The generation-input enumeration was extended in code but not in the normative text it is stated in — specs/00002-public-corpus-and-manifest/spec.md:183
  > FR-037 states the catalogue "MUST be a generation input digested into every entry built from it (FR-009b)", while FR-009b still enumerates only the equipment-category map, field-label vocabulary, and generation configuration. The two requirements contradict each other inside one spec.
  > FR-009b additionally carries "The enumeration is closed and MUST be extended in the same change that gives the generator a further committed input" — an obligation this amendment breached.
  > [RECURRING]: T068 corrected the five cited line numbers and left twelve more instances of the same defect class. Fix the class, not the citations.
  > Sites: spec.md 183, 231, 286, 358; data-model.md 251, 282, 283, 499; plan.md 102, 107, 109, 187; checklists/data-integrity.md 46 (dated evaluator note — lower severity, decide explicitly whether a closed checklist note is amended or left as a record).
  > Do NOT change: "the three validator-read supporting artifacts" at spec.md:222, data-model.md:476, plan.md:38, plan.md:225 — that phrase correctly names manifest.schema.json, retrieval-policy.json, exclusions.json.
- [X] T072 {SC-010} Remove the daemon-wide serving-image tag collision: resolve `procurement-api:e001-<checkout>` from one source used by both the workflow build and the check helpers, stamp `com.procurement.checkout` at build time, and report an image built by another checkout or carrying no stamp — tests/checks/helpers/images.py after:T071
  > Raised by QC, not by a requirement: two audit runs were misdiagnosed against an image a sibling checkout had built under the shared tag. The criteria say "the built request-serving image" and name no literal, so the tag is an implementation detail, as the host port was.
- [X] T073 {SC-009} Scope the orchestration stack to its own Compose project so `down -v` cannot destroy the development database, its volume, or the applied migration chain — tests/checks/test_orchestration.py after:T072
  > Raised by QC: the teardown twice destroyed the database mid-session and the following suites failed for reasons that looked like code defects. Verified end to end — a migrated database on 5436 is still healthy at alembic_version 0010 after the suite runs, and the checks project leaves no container behind.
