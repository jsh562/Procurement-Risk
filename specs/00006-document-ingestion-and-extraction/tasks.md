...
**Size**: 91 tasks over 74 requirements and six stories. This exceeds the skill's 5–10-per-phase target in US1 and US2; the epic is not split because every phase reads and writes the same six tables through one transaction boundary, and a sub-feature cut would put that boundary in two workspaces.
...
**Lands the gate, the block claim, the contracts, and the coverage wiring before any module or migration they constrain.**
...
- [ ] T034 [US1] {FR-071,FR-068} Build the closed-content-list report, failing on a missing item and on an empty population, in ingest/report.py → exports: build_report
...
- [ ] T058 [US3] {FR-032,FR-057} [COMPLETES FR-057] Record the declared floor 0.80 and the three deduction weights on the run row before the first document in ingest/runs.py after:T057
...
- [ ] T068 [US4] {FR-029} [COMPLETES FR-029] Assert the seeded page-split document cites the page printing the value and reassembles in page order in tests/ingest/test_page_split.py after:T066
...
- [ ] T073 [US5] {FR-055} Assert one active generation per document, zero superseded rows at commit, and zero rows left after a promotion in tests/schema/test_generations.py after:T072
...
- [ ] T078 [US6] {FR-056} Assert the five run-level kinds and the seven per-field outcomes intersect at zero by reading both CHECK definitions in tests/schema/test_failure_domains.py after:T077
...
- **Cross-phase edges**: T009→T001, T023→T021, T030→T029, T031→T029, T032→T031, T046→T032, T047→T012, T051→T008, T055→T040, T059→T012, T060→T057, T069→T009, T070→T011, T075→T070, T079→T054, T085→T013, T090→T005, T091→T051.
...
- **P1 boundary**: Phases 1–5 (T001–T065) are the viable deliverable — all 51 documents chunked, embedded, and citable, the 25 transmittals extracted with page citations and computed confidences, and untrustworthy values recorded as failures. Phases 6–9 are omittable without breaking any P1 criterion.
