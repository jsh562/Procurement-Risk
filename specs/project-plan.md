...
- **Single-import-site regression (E005; surfaced during E007's merge) — realized, and recorded rather than repaired.** `tests/checks/test_single_import_site.py` permits exactly one file under `/src` to name `project-vendor-roster`. Seven do, so the required check "Cross-entry checks, image assertions, and supply chain" has failed on `main` since `acfd1eb`. The count went 1→2 at `3bdc15a` and reached 7 by `915264a`, entirely within E005 — which had already repaired the identical breakage once, at `f732375`, by deriving the path from `model.roster.reader` instead of spelling it. Three offenders are generation-input manifest labels and three are test files, so this is name duplication rather than a second reader; the rule exists because the duplicate is the copy nobody remembers exists. The fix belongs to E005's code and E005 is ticked, so `main` stays red by decision rather than by oversight. Whoever repairs it must reduce the count to one rather than reorder — the companion assertion reads the first path-sorted mention, so any surviving offender under `model/procurement/` keeps it red. Mitigation for the class, not this instance: the check is cheap and already exists; what failed was noticing it, since the step is skipped whenever an earlier one aborts.
