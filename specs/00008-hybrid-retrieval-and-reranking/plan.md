...
**Audited against**: `project-instructions.md` **v1.2.11** (last amended 2026-07-29) ·
...
| **Governance — number claims (v1.2.11)** | **PARTIAL — one claim is not visible on the default branch** | v1.2.11 requires a number claim be visible where the allocator scans, and names this epic's `0022 → 0023` renumber as the measured cost of the old mechanism. Three of E008's four claim types comply: the **decision record** `specs/adrs/0023-local-inference-in-the-shared-gateway-package.md` is on `main`, its **catalog row** is in `specs/sad.md`, and E008 **claims no migration block** because it adds no migration. The fourth does not: **`specs/00008-hybrid-retrieval-and-reranking/` exists only on this branch.** Every other workspace — 00001–00007, 00009, 00010, 00012 — is on `main`. By the clause as written, 00008 is therefore unclaimed and an allocator "MAY take it regardless of what any unmerged branch asserts". **Residual risk is low but not nil**: the same clause fixes the convention that *a workspace number matches its epic number*, so an allocator working on E011 or E013 would take its own number rather than 00008. The exposure is a scan that allocates sequentially rather than by epic — which is exactly what caused the E002/E003 prefix collision this clause also records. Closed by landing the workspace directory on `main`, which merging this branch does; until then it is recorded rather than assumed away, because the cost of renaming mid-flight is what v1.2.11 calls the thing that cannot be done cheaply |
...
| FR-007 | Encoder guard | `src/gateway/src/gateway/inference/encoder.py`; **verification**: `src/gateway/tests/test_inference_session.py` | Refuse on identity mismatch, before any search |
...
| FR-015, FR-017, FR-018 | Reranker session | `gateway/inference/reranker.py`; **verification**: `src/gateway/tests/test_inference_reranker.py` (load once, warm at the fixed shape, rerank exactly the fused set) and `src/api/tests/retrieval/test_reranking.py` (SC-007's three zero-counters after readiness) | Load once, warm at max batch and sequence shape, rerank exactly the fused set |
...
| FR-021 | Readiness | `src/api/src/api/retrieval/readiness.py`; **verification**: `src/api/tests/retrieval/test_reranking.py` (readiness states and the degraded path) | Ready-degraded on load failure, never not-ready; caught inside lifespan (HINT-005) |
| FR-022 | Routes, readiness | `retrieval/routes.py`, `retrieval/readiness.py`; **verification**: `src/api/tests/retrieval/test_reranking.py` | *(corrected 2026-07-29 with FR-022's amendment)* Fusion-only stated in every **degraded** response body and at the readiness endpoint; every other unreranked response carries its own machine-readable reason (`arm_excludes_reranking`, `no_candidates_to_score`) and a statement that does not claim fusion-only, because a `lexical` or `dense` response is unreranked and is not fusion-only |
| FR-023 | Reporting | `retrieval/report.py`; **verification**: `src/api/tests/retrieval/test_reranking.py`, `test_report.py` | Every evaluation-facing output records the mode it ran in |
| FR-024, **SC-011** | Tests | `src/api/tests/retrieval/test_reranking.py` | Forces the failure **at the artifact-loading boundary** — an absent, unreadable or digest-mismatched graph, so the exception arises where FR-016's verification runs — and asserts all three observables FR-024 enumerates: ready-degraded, the fusion-only statement with results still returned, and the mode on the evaluation-facing output. Setting the flag directly does not discharge it |
...
| FR-027, FR-028 | Connection, arms | `src/api/src/api/db.py`, `retrieval/arms.py`; **verification**: `src/api/tests/retrieval/test_arms.py` (integration, live index) | Breadth on the connection (AD-002); strict iterative scan (AD-003) |
| FR-029 | Parameters | `retrieval/parameters.py`; **verification**: `src/api/tests/retrieval/test_report.py` | Emitted with any result an evaluation consumes |
...
| FR-037 | Fusion query, arm selection, reranker session | `retrieval/fusion.py`, `retrieval/arms.py`, `src/api/tests/retrieval/test_arms.py` | The derived constraint: depth 50 → reranked count 50 → breadth ≥ 50, and the top 50 of the fused ordering are what the reranker scores when the fused set holds up to 100 |
| FR-038 | Configuration, reranker session | `src/api/src/api/config.py`, `gateway/inference/reranker.py`, `src/gateway/tests/test_inference_threads.py` | Intra-op from the container's CPU quota, inter-op one; asserted set rather than defaulted |
| FR-039 | Connection, vector settings | `src/api/src/api/db.py`, `src/api/tests/retrieval/test_single_statement.py` | Extension version verified against the pinned digest before iterative scan is relied on; version recorded with the settings |
| FR-040 | Connection, parameters | `src/api/src/api/db.py`, `retrieval/parameters.py`, `src/api/tests/retrieval/test_single_statement.py` | Breadth recorded; any value above FR-027's floor is a recorded change carrying a re-measured latency figure |
| FR-041 | Readiness, reporting | `retrieval/readiness.py`, `retrieval/report.py`, `src/api/tests/retrieval/test_reranking.py` | Degraded-path latency reported on FR-033's terms and asserted not slower than the reranked path |
...
+                                                      test_router.py, test_reranking.py,
+                                                      test_part_number_coverage.py,
+                                                      test_results.py, test_page_provenance.py,
+                                                      test_arms.py, test_flag_parity.py,
+                                                      test_report.py, test_ordering_digest.py,
+                                                      test_metrics.py, test_evaluation_set.py,
+                                                      test_gate_measurement.py,
...
+ src/gateway/tests/test_inference_session.py        the encoder guard and the one shared
+                                                      vector space
+ src/gateway/tests/test_inference_reranker.py       both graphs, joint scoring, truncation
+ src/gateway/tests/test_inference_threads.py        FR-038's counts, read back from the
+                                                      created session rather than the options
