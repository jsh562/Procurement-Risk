...
| `extracted_value_id` | `uuid` | NULL | `fk_rem__extracted_value … ON DELETE RESTRICT`; `uq_rem__extracted_value UNIQUE (extracted_value_id)` |
| `po_line_id` | `uuid` | NULL | `fk_rem__po_line … ON DELETE RESTRICT`; `uq_rem__po_line UNIQUE (po_line_id)` |
...
| `resolved_entity` | `resolved_entity_id` PK; UK `(normalized_manufacturer, normalized_part_number)` | 1:N `resolved_entity_member` | **P2.** Confirmed cross-document identity; agreement attributes as `text[]` (TR-034). |
| `resolved_entity_member` | `member_id` PK; UNIQUE `extracted_value_id`; UNIQUE `po_line_id` | N:1 `resolved_entity` (CASCADE); XOR to `extracted_value` / `purchase_order_line` | **P2.** Plain `UNIQUE` (NULLS DISTINCT) prevents a record joining two entities. The only sanctioned join between values and lines; E009 populates it (TR-035, TR-045). |
