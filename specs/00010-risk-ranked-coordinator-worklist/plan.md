...
|---|---|---|
| *Audited against* | `project-instructions.md` **v1.2.8** — re-audited 2026-07-29 | — |
| *Prior audit* | v1.2.4, superseded while this branch was in flight by v1.2.5 through v1.2.8 — four revisions of one new **Temporary Files** rule, the last of which withdrew part of an earlier one. Re-audited twice for that reason: once at v1.2.7 (T074) and again at v1.2.8 when E006's merge brought the withdrawal. Governance requires a feature whose recorded audit names a superseded version to re-run its compliance gate before its next phase gate, because "an amendment moves the ground under every epic already in flight, and their branches keep validating against the version they were cut from". Every row below was re-checked; only the new rule changed a verdict | — |
...
| Source Code Layout | `/src/web` and `/src/api` only, no new entry. Entry-local tests live in `src/api/tests/` and, for the web tier, beside the module they cover in `src/web/app/worklist/` — `vitest.config.ts` records the choice ("a module's own tests sit beside it"), and `src/web/__tests__/` holds only E001's cross-cutting boundary test. Never under root `/tests`, which is reserved for cross-entry verification and whose committed checks reject an `api.*` import outright | PASS |
...
| Performance | pytest-benchmark against the frozen fixture | Worklist p95 ≤ 1.5 s on one shared vCPU, the registered target (SC-017, SC-018) | Real Postgres, the frozen fixture's 16 lines (see § Performance measurement conditions) | `uv add --dev pytest-benchmark`; needs a `Performance benchmark (api)` CI step |
...
> **Paths corrected 2026-07-29 (QC iterations 3 and 4).** Five entries in this document named files that were planned and never created — `test_no_datastore_from_web.py` and `__tests__/worklist.test.tsx`. The work exists; it landed beside the code it asserts over rather than at the planned path. T071 corrected the three entries in `tasks.md` and left these five, which is the same half-finished shape the task was filed for. Iteration 4 then found that two of the five replacements were themselves wrong — a `.tsx` extension on a `.ts` file, and a glob that does not match it.
...
| Row composition, region assignment, sort-key set, state copy distinctness (FR-026, FR-027, FR-044) | `app/worklist/{Row,WorklistBoard,page}.test.tsx` and `stateCopy.test.ts` | `Unit tests (web)` | runs today |
...
src/web/
+   app/worklist/*.test.{ts,tsx} # row composition, region assignment, copy distinctness
