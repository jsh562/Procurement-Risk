...
- **FR-009**: A SYNTHETIC entry MUST additionally carry generator identity, the committed seed, generation date, a content hash for every generation input it was built from, its document-model hash, and its irregularity classes; it MUST NOT carry a source location, issuing body, retrieval timestamp, upstream digest, or third-party revision date, because a generated document has no such origin and recording one would be a provenance falsehood.
- **FR-009b**: The generation-input hashes MUST cover every committed artifact the generator reads — the project/vendor roster, the equipment-category map, the field-label vocabulary, and the generation configuration — recorded as a mapping from repository-relative path to content hash. Corpus validation MUST recompute each and fail naming any input that has drifted, together with every document generated from it. Without this, loosening a supporting artifact moves no digest and fails nothing, which leaves the corpus asserting a provenance its inputs no longer support.
...
- **SC-023** [US3]: 100% of committed generation inputs carry a recorded content hash in every entry generated from them, and a change to any one of them causes validation to name that input and fail.
...
Audited against `project-instructions.md` **v1.2.0** and the governance rules in `AGENTS.md`. Re-run 2026-07-25 under the Governance rule that a feature audited against a superseded version re-runs its gate; the prior audit named v1.1.3. Verdict **PASS** — no instruction violation. Principle verdicts: I PASS, II N/A, III PASS, IV PASS, V PASS, VI PASS (evaluation sets excluded to E014 by an explicit scope decision), VII PASS, VIII N/A; Technology Stack PASS; Testing & Quality Policy deferred to Plan; Source Code Layout PASS; Data Provenance PASS; Governance PASS.
