...
- [ ] T014 [US1] {FR-005,FR-006,FR-041} Derive miss mass both directions, the beyond-horizon bound and absence at the anchor in src/api/src/api/compute/distribution.py after:T013 (AD-005, HINT-004)
...
- [ ] T028 [US2] {FR-024,FR-025} Aggregate the census by document with its denominator, target and no-interval declaration in src/api/src/api/risk_read/source_binding.py after:T027 (AD-009)
...
- [ ] T040 [US4] {FR-031} Reconcile each value against the fit and withhold under covariate_withheld on mismatch in src/api/src/api/risk_read/covariates.py after:T039 → exports: reconcile_covariates()
