...
| Technology Stack | ReportLab, Pillow, pdfplumber, and `jsonschema` are additions to `/src/model`'s manifest, not stack changes — they sit inside the declared Python boundary and bring no web framework (AD-001, AD-004, AD-005). **Partial on Infrastructure**: the section states "one-shot jobs under a non-default profile", which the job-invocation change departs from; ADR-0011 supersedes ADR-0003 on that clause and the section needs amending to match | PARTIAL |
...
| Data Provenance | Public-domain or synthetic only; per-document manifest entry; copyrighted standards cited never included (FR-005); no mixed licenses per basis identifier within a location (FR-013); datasheet ships with the synthetic layer (FR-027). **Partial**: the section mandates source, issuing body, and retrieval date on *every* document, which FR-009 makes a validation failure on the ≥25 synthetic documents. The substitution is required by Principles I and III — a generated document has no issuing body, and recording one would be a fabrication — but it conflicts with the registered wording and must be amended rather than assumed | PARTIAL |
| Governance | Three open items, none resolvable from inside this plan. **F5**: `specs/project-plan.md` still describes E001's workflow as dispatch-only and assigns both triggers to E002. **Provenance wording**: `project-instructions.md` Data Provenance, `specs/sad.md` Data Management, and `specs/project-plan.md` E002 acceptance criterion 2 all state the universal five-field form and need the layer-dependent form. **Infrastructure wording**: `project-instructions.md` Technology Stack and `specs/sad.md` Deployment view state container-profile job invocation, superseded by ADR-0011. All three require `/sddp-amend`; the instructions changes additionally require a version bump with an ISO-dated changelog entry | PARTIAL |

## Complexity Tracking

Two registered-document deviations that this plan requires and cannot resolve from inside itself. Both are corrections to wording that predates the case at hand, not requests to relax a principle.

| Violation | Why Needed | Simpler Alternative Rejected Because |
|-----------|-----------|--------------------------------------|
| Data Provenance mandates source, issuing body, and retrieval date on *every* corpus document; this feature makes all three a validation failure on the ≥25 synthetic documents | A generated document has no issuing body and no retrieval date. Recording one would be a fabricated provenance claim — the exact failure Principle I forbids and Principle III tells us to record as absent rather than store wrong | Populating the fields with placeholders was rejected because a placeholder is indistinguishable downstream from a verified value, which is the property the manifest exists to guarantee. Making the fields optional was rejected because optionality reintroduces the silent-default path for the real layer too |
| Technology Stack → Infrastructure states one-shot jobs run under a non-default Compose profile; corpus generation and validation run as console entry points instead | A model job image cannot share the `./src` build context: `src/.dockerignore` admits only `api` and `gateway`, and two build-gating E001 contracts assert `model` is absent from it. Honouring the profile would mean deleting contracts that exist to keep the modeling boundary out of the serving build | A BuildKit per-Dockerfile ignore file admitting `model` for one Dockerfile was rejected — it leaves two divergent context policies for one directory and buys a container surface nothing in this epic requires. Recorded as ADR-0011 superseding ADR-0003 rather than as a feature-local choice |
...
| CorpusManifestEntry | Common: `location`, `layer`, `license_basis`, `content_hash`. REAL adds `source_location`, `retrieval_response_status`, `retrieved_at`, `issuing_body`, `masterformat_section`, `agency_variant`, `revision_date`, `upstream_digest`. SYNTHETIC adds `generator_id`, `seed`, `generation_date`, `roster_hash`, `document_model_hash`, `irregularity_classes[]` | 1:1 with one Document; belongs to 1 CorpusManifest | The asymmetry is enforced, not conventional: a SYNTHETIC entry carrying any retrieval field fails validation rather than holding a blank. `masterformat_section` stores the bare number so agency variants count once toward coverage |
...
**Supporting artifacts** (validator inputs, outside every location, covered by no `content_hash`): `manifest.schema.json`, `retrieval-policy.json`, `exclusions.json`, `generation-config.json`, `equipment-category-map.json`, `field-label-vocabulary.json`.
