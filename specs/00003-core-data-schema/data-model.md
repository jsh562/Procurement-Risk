...
| TR-034 | `resolved_entity`, `resolved_entity_member` — extended by E009's `0505` with `resolution_run_id` and `project_id` on the entity and `resolution_run_id`, `specification_section` and generated `member_record_id` on the member ({SAD:ADR-0023}, P-6) |
| TR-035 | `uq_rem__extracted_value`, `uq_rem__po_line` — **dropped by E009's `0505` and re-created run-scoped** as `uq_rem__run_extracted_value`, `uq_rem__run_po_line`. The rule holds per run rather than globally and still depends on `NULLS DISTINCT` ({SAD:ADR-0023}, P-6) |
...
| TR-083 | This document — every table, column, constraint, index, and seeded row above is normative. **Amended 2026-07-29 (P-6):** the requirement now admits a later epic's additive extension of a table E003 created under {SAD:ADR-0023}, documented in the extending epic's own data model and *admitted* here; E003 itself still may create no object absent from this document. §`resolved_entity` / `resolved_entity_member` › Extended by E009 is that admission |
