...
| `generation_inputs` | list | **Exactly two entries**, each `{path, digest, digest_kind}`, repository-relative: `data/roster/project-vendor-roster.json` with `digest_kind: canonical_content`, and `data/corpus/synthetic/equipment-category-map.json` with `digest_kind: raw_bytes`. The kind is recorded per entry because the two are hashed by different conventions (G-3) and a reader of the committed artifact would otherwise have no way to tell which digest to recompute. A missing entry is an error, never an omission. | computed |
...
| L-5 | No manufacturer or part-number overlap with E002's corpus, so **this dataset supports no cross-document identity resolution on the key E009 blocks with**. Reversal: E002 publishes those fields. Alternative: a corpus carrying product data rather than transmittal fields alone | FR-016 minimum |
...
| # | Gap | Why | Covered by |
|---|---|---|---|
| G-1 | E009's blocking key finds no true pairs on `manufacturer` / `part_number`, and finds them on `material_item` only after the tag is normalised out | E002 publishes neither field, and publishes no item inventory to match a tag against | **Integration obligation on E009** — raise in the plan. FR-034 / SC-026 remain pending and outside the completion denominator; L-5 and L-9 carry it to the datasheet's reader. **Unblocking trigger, observable and owned**: the condition is "E002's published field vocabulary contains a manufacturer field *and* a part-number field", owned by whoever owns E002 (the corpus amendment) with E006 and E009 as the co-signatories the spec's amendment-need paragraph names. E005 owns the *detector*, not the amendment: a build-gating check asserts that E002's published vocabulary contains neither field, and **fails when it gains them** — so the blocked state cannot persist unnoticed once the trigger fires, and the failure names FR-034 and SC-026 as the two obligations to re-evaluate. Recorded as a check rather than a note because a gate nobody watches is how a blocked criterion becomes a permanent one |
...
| FR-034 | §Not achievable — modelled as generated, **not** as corpus-overlapping; excluded from the completion denominator; limitation L-5; gap G-1 |
...
| FR-037 | §Line record — the `manufacturer` grammar, whose stem suffixes and trade nouns are disjoint from E001's vendor convention, and whose vocabulary is a closed tuple in generator source rather than a third generation input; DV-021 |
