...
- **SC-008** [US2]: 100% of **attempted field extractions** resolve to either a stored value carrying a page citation and a per-field confidence, or a failure record carrying one outcome from the closed set; zero attempts are unaccounted for. **Denominated on attempts (FR-069), not on stored rows.**
...
- **SC-054** [US3]: Attempted field extractions equal stored values plus failure records exactly, zero attempts unaccounted for; 100% of published counts and rates name their unit.
...
- Q: Which fields are attempted, given ten of the twenty-two vocabulary terms cannot appear on a transmittal? → A: Only the declared transmittal subset, per chunk, with a field absent from a whole document recorded once per document (FR-058).
...
| Source Code Layout | PASS | Modeling-entry console entry point, gateway-mediated provider access |
| Development Workflow | PASS | Workspace `00006-…` matches epic E006; branch cut to match |
