...
- Decision record numbers are claimed at epic start, exactly as migration numbers are. — Numbers are allocated by scanning for the highest already in use, so two epics branching from one baseline will otherwise allocate the same number and both be right.
