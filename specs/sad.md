...
| ADR-0013 | Schema Ownership in the Modeling Entry with Constants Published Through the Database | accepted | 2026-07-25 | — | [0013-schema-ownership-in-the-modeling-entry.md](adrs/0013-schema-ownership-in-the-modeling-entry.md) |
...
- Schema ownership fixed: `/src/model` owns all migrations and is the only entry declaring a database client, while shared constants are published in a single-row table the serving boundary reads over the connection rather than importing — keeping the four-entry rule and ADR-0010 intact ({SAD:ADR-0013}).
...
- **Scope of the single-database-client rule, as read by E004.** {SAD:ADR-0013} fixes ownership of the schema, the DDL, and the migration tooling in `/src/model`. It is read as *not* forbidding every other entry from holding a Postgres driver — `/src/api` demonstrably holds one, since ADR-0013 has it reading `schema_constants` over a connection and {SAD:ADR-0004} has it computing risk in SQL at request time. The gateway therefore keeps a driver for the sole purpose of writing invocation records, which {SAD:ADR-0010} already lists among its sanctioned contents. This is an interpretation of one sentence in ADR-0013's consequences that reads more broadly than its drivers and options do; it is recorded here so E005–E007 inherit a stated reading rather than an ambiguity, and a superseding record should settle it properly.
