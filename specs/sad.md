...
| ADR-0019 | Ingested Derived Data Carries an Active or Superseded Generation | superseded | 2026-07-27 | — | [0019-ingested-derived-data-carries-an-active-or-superseded-generation.md](adrs/0019-ingested-derived-data-carries-an-active-or-superseded-generation.md) |
| ADR-0020 | Superseded Generations Are Removed at Promotion, Not Retained | accepted | 2026-07-27 | ADR-0019 | [0020-superseded-generations-are-removed-at-promotion-not-retained.md](adrs/0020-superseded-generations-are-removed-at-promotion-not-retained.md) |
...
- Derived ingested data carries a generation that is active or superseded, held per document rather than per run because a run re-ingests only the documents whose inputs changed ({SAD:ADR-0019}). At most one active generation per document is a partial unique index rather than application discipline, so two live generations are unrepresentable; downstream readers filter through a view rather than each remembering to. Retirement deletes strictly leaf-up, because `ON DELETE RESTRICT` cannot be deferred the way `NO ACTION` can. **Superseded generations are removed at promotion rather than retained** ({SAD:ADR-0020}, superseding {SAD:ADR-0019} on that clause only): chunk ordinals are unique within a *document*, not within a generation, so two retained generations for one document are unstorable. Reverting a bad promotion is therefore a re-run of the previous chunker version, not a flag flip, and two chunker generations cannot be compared inside one database.
