...
**Tests**: Included. Strict red-green-refactor is **mandatory for one module only** — `retrieval/metrics.py`'s pure scoring functions, by spec FR-042 and the quality policy: the Wilson interval, the percentile bootstrap, and the overlap verdict. Each appears as an ordered pair (T041→T042, T043→T044, T045→T046) whose test task precedes its implementation task and whose completion condition is an **observed failure**, never a green suite. The SQL-resident fusion ranking is the opposite case: FR-002 leaves it no pure function, so its property obligation is discharged by the **pseudo-oracle** of `plan.md` §Testing Strategy under that section's four soundness conditions. No red-green pair is emitted for it. Every other module here is test-after.
...
**Size**: 95 tasks over 46 requirements, 16 success criteria and six stories. US1 carries 28, which exceeds the skill's 5–10-per-phase target. The epic is not split: SC-001 and SC-002 are US1 criteria and are measured by `metrics.py` over E008's own frozen set, so cutting the measurement surface into a sub-feature would put a P1 criterion outside the phase that owns it.
...
- **Ordering constraints that shape the phases — each is a declared edge, not a note**:
  - **Four amendments gate the epic.** T001–T004 are the verifiers, each closed by citing the amending revision on the default branch (SC-015). T003 is the one this design cannot proceed without: `project-instructions.md` §Source Code Layout is the clause ADR-0022 contradicts. T005, T006, T018 and T012 declare `after:T003`/`after:T004` and every later chain reaches the gate through them.
...
**Lands the four amendment gates, the dependency relocation, both mirrored constants, and the two workflow lines before any module they constrain. Nothing below Phase 1 may be started while T001–T004 are open.**
...
- [ ] T003 {FR-044} Confirm project-instructions.md §Source Code Layout excepts a shared inference runtime — the clause ADR-0022 contradicts — and cite the revision in plan.md item 3 (SC-015)
- [ ] T004 {FR-045} Confirm specs/sad.md's ADR catalog carries the ADR-0022 row appended after ADR-0021 and cite the revision in plan.md §Pending Amendments item 4 (SC-015)
- [ ] T005 Move onnxruntime and tokenizers from src/model/pyproject.toml to src/gateway/pyproject.toml so model inherits them through the gateway (ADR-0022) after:T003
- [ ] T006 {FR-002} Extend the forbidden contract in src/api/pyproject.toml to name api.retrieval as the third computation package, and declare the retrieval dependencies after:T003
...
- [ ] T008 Admit numpy and narrow heavy to pymc, arviz, pandas in tests/checks/test_dependency_isolation.py — the mirrored copy, TR-003/TR-004 (HINT-003) after:T007
...
- [ ] T017 {FR-016} Write the one-off dynamic quantization emitting the INT8 graph with generator identity, seed, date and source hash in src/model/tools/quantize_reranker.py after:T016
...
- [ ] T022 Repoint src/model/src/model/ingest/embed.py at gateway.inference.encoder so exactly one pooling implementation exists (ADR-0022) after:T021
...
- [ ] T024 [US1] {FR-001,FR-002} Author the two-CTE reciprocal-rank fusion statement, one full outer join, in src/api/src/api/retrieval/fusion.py after:T015 → exports: fuse_candidates
- [ ] T025 [US1] {FR-003,FR-037} Fetch 50 candidates per arm and cut the fused ordering to the reranked count of 50 in src/api/src/api/retrieval/fusion.py after:T024
- [ ] T026 [US1] {FR-004} Apply the tie-break key inside each arm's CTE as well as in the final ordering, in retrieval/fusion.py (AD-001, HINT-001) after:T025
- [ ] T027 [P] [US1] {FR-001} Write the pseudo-oracle property tests from the published definition over generated per-arm rank vectors in src/api/tests/retrieval/test_fusion_oracle.py
- [ ] T028 [P] [US1] {FR-002} [COMPLETES FR-002] Capture the statements one search executes and assert exactly one ranking statement with no SET in src/api/tests/retrieval/test_single_statement.py
- [ ] T029 [P] [US1] {FR-003} Assert over EXPLAIN that each arm's LIMIT survives CTE inlining and each node reports the fetch depth in src/api/tests/retrieval/test_fusion_plan_shape.py
- [ ] T030 [US1] {FR-003} [COMPLETES FR-003] Assert the candidate set at the 50-row cut, with a tie engineered at the last in-window position, in test_candidate_set.py after:T023
- [ ] T031 [US1] {FR-020} Assert identical ordering twice, under seven flipped planner settings whose EXPLAIN plan differs, and across an exact-path rebuild, in test_determinism.py after:T030
...
- [ ] T034 [P] [US1] {FR-008} Scan construction sites and assert the private factory is the only one (AD-004) in src/api/tests/retrieval/test_page_provenance.py
...
- [ ] T036 [US1] {FR-006} Assert over the emitted artifacts that no figure or label names BM25 and the no-corpus-statistics statement is present, in retrieval/report.py and test_report.py after:T035
...
- [ ] T045 [US1] {FR-032,FR-042} Write failing property tests for the overlap verdict — symmetry, reflexivity, touching endpoints — in test_metrics.py, done only on an observed failure after:T044
...
- [ ] T047 [US1] {FR-031} [COMPLETES FR-031] Record the method on every emitted interval and assert no non-proportion statistic carries wilson, in retrieval/report.py and test_metrics.py after:T046
- [ ] T048 [US1] {FR-042} [COMPLETES FR-042] Assert the generated domains reach set size one upward, all-hit, all-miss, ties and endpoints, and refuse an empty set, in test_metrics.py after:T047
...
- [ ] T051 [US1] {FR-043} Perturb a copy of the committed set and assert the harness exits non-zero before any measurement, in src/api/tests/retrieval/test_evaluation_set.py after:T050
...
- [ ] T095 [US2] {FR-046} Bound the result array by rule — at most `limit` ranked plus one deterministic match per recognised token, query length capped — and assert the bound in src/api/src/api/retrieval/results.py and contracts/openapi.yaml after:T055
- [ ] T056 [US2] {FR-010} [COMPLETES FR-010] Verify the pattern against the part numbers enumerated from the generator's pre-render document model in test_part_number_coverage.py after:T055
- [ ] T057 [P] [US2] {FR-011,FR-012,FR-014} Assert fall-through, the additive union against the route-disabled result, and arm_excludes_route on the single-arm paths, in test_router.py
...
- [ ] T068 [US3] {FR-033} Assert the report carries workload, environment, measurement point, occasion, counter, arm and corpus size, in src/api/tests/retrieval/test_performance_report.py after:T067
...
- [ ] T073 [US3] {FR-036} [COMPLETES FR-036] Assert the selection rule over constructed figures, including the overlapping-interval case where both are reported, in test_metrics.py after:T072
...
- [ ] T080 [US4] {FR-024} Force load failure at the artifact-loading boundary and assert ready-degraded, the fusion-only statement with results, and the recorded mode, in test_degraded.py after:T079
- [ ] T081 [US4] {FR-041} Report the degraded path's per-query latency on FR-033's terms and assert it never exceeds the reranked path's over the same query set, in test_degraded.py after:T080
...
- [ ] T084 [US5] {FR-025} [COMPLETES FR-025] Assert each of the five arms returns independently and identically across two runs on an unrebuilt index, in test_arms.py after:T083
- [ ] T085 [US5] {FR-037} [COMPLETES FR-037] Assert the derived constraint — depth 50, reranked count 50, breadth at or above 50, top 50 of a fused set of up to 100 — in test_workload.py after:T084
...
- [ ] T088 [US6] {FR-027,FR-028} Assert the connection-borne breadth is at or above the fetch depth and a filtered query returns the requested count, in test_vector_settings.py after:T087
...
- [ ] T090 [US6] {FR-026} [COMPLETES FR-026] Build two differently configured application instances and compare the observable set FR-026 enumerates, in test_flag_parity.py after:T089
...
- [ ] T093 [P] Verify the architecture contracts stay green — api.retrieval forbidden, numpy admitted in both copies, the serving image's contents after:T009
...
- **The four amendment gates precede everything.** Each phase's first module-creating task declares the edge, and within a phase sequential `T###` order carries it — the convention stated at the foot of this section. **The claim is that shape, not that all 95 tasks name a gate**: 29 do not, including T015's fusion chain, and an earlier draft of this line asserted otherwise. Concretely: T003 blocks the relocation and the gateway package (T005, T006, T018 declare `after:T003`); T004 blocks the first database task (T012 declares `after:T004`); T001 blocks the metrics red-green chain (T041 declares `after:T001`), because until the `specs/prd.md` amendment lands a registered document specifies a Wilson interval on MRR and FR-031 forbids one; T002 blocks the lexical-arm disclosure (T035 declares `after:T002`), because the inert field weighting has no recorded owner until it lands. T010 declares `after:T003` because it writes the `gateway/inference` path into CI that T003 must legalise. Under phase order plus these edges, no module, session, statement or figure is built while any of T001–T004 is open — but a purely edge-driven executor must respect phase boundaries to get that guarantee.
...
- **The two mirrored constants move together by edge.** T008 declares `after:T007`. Only `numpy` is admitted; T005 removes `onnxruntime` and `tokenizers` from the denylist by relocating the declarations, and adding them explicitly trips the staleness assertion that every excluded name is still declared by `model` (HINT-003, ADR-0022 §Negative). T009 then reflects the admitted runtime in the image assertions, `after:T008`.
- **Mandatory red-green pairs** (spec FR-042, quality policy): T041→T042, T043→T044, T045→T046. Each test task is complete only when its suite has been run and **observed to fail** — a collection error for the absent module in the first pair, a missing-symbol failure in the second and third — with the observation recorded on the task line. Neither member of a pair is ever `[P]`. No such pair exists for fusion ranking: FR-002 leaves it no pure function, and its property obligation is discharged by T027's pseudo-oracle under the four soundness conditions in `plan.md` §Testing Strategy — derived not transcribed, generated not read-back inputs, an enumerated uncovered surface, and adjudicated disagreement.
...
- **The integration tier seeds its own corpus.** T023 commits and seeds the fixture because the merge gate applies migrations and never ingests. T030, T031, T084, T088 and T090 all read it; T030 carries the declared edge `after:T023` and the rest reach it through their own chains.
- **The benchmark module is registered before it is written.** T011 names `src/api/tests/test_retrieval_benchmark.py` in the `Performance benchmark (api)` step; T069 declares `after:T011`. The api unit step runs `-m "not benchmark"`, so a benchmark module absent from that step's file list runs nowhere at all.
...
- **Declared edges**: T005→T003, T006→T003, T007→T005, T008→T007, T009→T008, T010→T003, T011→T010, T012→T004, T014→T012, T017→T016, T018→T003, T019→T018, T020→T018, T021→T020, T022→T021, T024→T015, T025→T024, T026→T025, T030→T023, T031→T030, T033→T032, T035→T002, T036→T035, T037→T015, T038→T037, T039→T021, T040→T039, T041→T001, T042→T041, T043→T042, T044→T043, T045→T044, T046→T045, T047→T046, T048→T047, T049→T044, T050→T049, T051→T050, T053→T052, T054→T053, T055→T054, T056→T055, T095→T055, T058→T018, T059→T058, T060→T059, T061→T060, T062→T061, T063→T062, T064→T063, T065→T064, T067→T066, T068→T067, T069→T067, T070→T064, T071→T046, T072→T071, T073→T072, T074→T064, T075→T074, T076→T075, T077→T076, T078→T077, T079→T078, T080→T079, T081→T080, T082→T063, T083→T082, T084→T083, T085→T084, T086→T082, T087→T086, T088→T087, T089→T088, T090→T089, T091→T090, T092→T091, T093→T009, T094→T091.
- **Symbol-import edges gate execution exactly as `after:` does**: T014←T013:RetrievalConfig, T040←T032:RetrievalResult, T066←T062:TruncationReport. A consumer honouring only `after:` under-constrains them.
- **P1 boundary**: Phases 1–6 (T001–T081) are the viable deliverable — a coordinator's question reaches ranked passages carrying their page, part numbers resolve deterministically and additively, the fused set is reranked by a warmed cross-encoder, and a system without one says so. Phases 7–9 are omittable without breaking any P1 criterion, at the cost of E014's ablation table and SC-012/SC-013/SC-014.
- Tasks marked `[P]` touch distinct files and carry no `after:T###` or `← T###:` edge to another task in the same batch. Within a phase, sequential `T###` order already implies the dependency where no `after:` is written — T027–T029 read the statement T024–T026 build, and T034 scans the module T032–T033 create.
