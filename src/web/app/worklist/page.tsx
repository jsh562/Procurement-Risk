 ...
 * failed to load them.
 *
 * Phase 2 renders the no-active-run state completely and lists every open line
 * with its identity and need-by date. Ranked rows with their figures arrive
 * with US1; this file's shape is what they extend.
 */
  ...
  return (
    <main className={styles.page}>
      ...
      {worklist.counts.total === 0 ? (
        ...
        <UnrankedGroup rows={worklist.unranked} total={worklist.counts.total} />
      )}
    </main>
  );
}
 ...
 *
 * FR-019 and FR-038. The as-of date is row text reachable without hover or
 ...
 * further. Not criticality, not calendar margin — with the risk figures
 * withheld those would be the only numbers on the row and would be read as risk.
 */
function UnrankedGroup({
  rows,
  total,
}: {
  readonly rows: readonly UnrankedRow[];
  readonly total: number;
}) {
  return (
    <section className={styles.group} aria-labelledby="unranked-heading">
      <h2 id="unranked-heading" className={styles.groupHeading}>
        Open lines ({rows.length} of {total})
      </h2>
      <ul className={styles.rows}>
        {rows.map((row) => (
          <li key={row.po_line_id} className={styles.row} data-state={row.state}>
            ...
            {/* FR-050. The row's resolved state as text, not as a tint. */}
