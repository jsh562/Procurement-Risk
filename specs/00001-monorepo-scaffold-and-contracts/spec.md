...
- A workflow that runs every check in one place, automatically on push and on demand by dispatch
...
**Deliverables**:
- A workflow invoking every check, triggered automatically on push and available on manual dispatch
- Per-check reporting so a failure names which contract or assertion failed

**Validation Criteria**:
1. **Given** a clean tree, **When** a commit is pushed, **Then** every check runs automatically and the workflow succeeds.
2. **Given** any single contract violated, **When** the checks run, **Then** the run fails and the output names the violated check.
...
- **TR-020**: The system MUST provide a workflow invoking every check and reporting per-check results, triggered both automatically on push and by manual dispatch — the push trigger so a contract violation fails the build rather than raising a review comment, and the dispatch trigger so TR-022's violation-injection input can be exercised on demand. The push trigger is the one acceptance depends on; dispatch is a convenience. No check it invokes requires credential access — each is a static analysis, a test over committed fixtures and pure functions, or a local image build and run against public images — so the dispatched run requires no repository secret, and its secret surface is knowable from this requirement rather than only from the workflow file. A later epic adding a check that does require one MUST state that check's secret surface.
- **TR-021**: The workflow file MUST be landed on the default branch before this epic closes, so pushes to that branch are gated and SC-013's clean run is evidenced during the epic rather than deferred past completion. Landing it also makes the workflow dispatchable, but dispatchability is a convenience rather than the reason for the requirement.
- **TR-022**: The system MUST provide an evidence path for a violated run that does not require committing a violation to a production contract root — either a throwaway branch pushed solely to capture the failing run and then deleted, or a workflow input that writes a violation into the runner's ephemeral working tree. Without this, TR-007's design makes every committed violation a passing test, so no run could ever fail. Any injected violation MUST be lint-clean while still breaking its contract: a bare unused import is reported by the linter several steps before the contracts execute, so the run fails naming the linter rather than the violated contract, and a run that fails for the wrong reason is not evidence that the contract works.
...
- The repository has no commits yet. This epic establishes the first commit and a hosted remote with a default branch, both of which the workflow requires before it can gate anything; until they exist, no history-dependent tooling can be assumed to work.
...
- **SC-013** [OBJ7]: With the workflow landed on the default branch, a push-triggered run on a clean tree passes every check; and a run with any single contract violated fails and names the violated check. The violated half is evidenced two ways, both automatic: the committed negative fixtures (TR-007, SC-004) plant a real violation against each contract and require it to break, and they execute inside the workflow on every push; and a throwaway branch pushed under TR-022's pushed-branch path produces a real failing run naming the violated check. Neither requires a person to remember to trigger anything — which is why this criterion no longer names manual dispatch as its evidence path. The dispatch trigger remains available for on-demand use and is not an acceptance gate.
...
### Session 2026-07-25 (QC iteration 3 — OBJ7 evidence path)

- Q: OBJ7's criteria required a *dispatched* run as evidence. Two dispatches ran: a clean one succeeded, and one carrying an injected violation failed at "Lint (Python)" rather than at the contract, because the injected bare import is reported as an unused import several steps before the contracts execute. Should the payload be corrected and the dispatch re-run, or should the criteria name the evidence that already exists? -> A: Amend the criteria (OBJ7 VC1, VC2, SC-013), and correct the payload as well (TR-022).
- **Why this is not a target adjusted to match a result** (Principle VII): the requirement is unchanged — a contract violation fails the build, and the failure names the violated check. That property is already proven *more* thoroughly than the criterion asked. The committed negative fixtures (TR-007, SC-004) plant a real violation against each of the three contracts, including the re-export laundering path that passes the import contract, and they execute inside the workflow on **every push**. A throwaway branch pushed under TR-022's pushed-branch path additionally produced a real failing run naming the violated check. What changed is the *mechanism used to observe* the property: from a one-off manual dispatch to continuous automatic evidence. The bar rises, because automatic evidence cannot be forgotten and a manual click can.
- What a manual dispatch uniquely proved was narrower than it appeared: that the `workflow_dispatch` input path can inject a violation — a property of the test harness rather than of the system. Requiring it per epic would have made every future epic inherit a human step that demonstrates nothing the fixtures do not.
- The dispatch trigger and its injection input are retained and their payloads corrected under T071; they are simply no longer an acceptance gate.
