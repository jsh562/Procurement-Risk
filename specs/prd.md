...
|--------|--------|----------------|--------------------|
| Retrieval recall@5 | ≥ 0.85 on the frozen 50-item evaluation set, published with a Wilson 95% confidence interval on the proportion of queries returning a relevant chunk in the top 5 | Top-5 is what the interface surfaces; the interval prevents overclaiming on a small set. Wilson is valid here because the quantity *is* a binomial proportion — 50 queries, each a hit or a miss | Every evaluation run; gate at P1 release |
| Retrieval MRR | ≥ 0.70, published with a percentile bootstrap 95% interval resampled over the 50 queries, reported with its resample count and its seed | Rank quality, not just presence, determines whether the coordinator finds the answer. **The interval method is not interchangeable with the row above.** MRR is the mean of 50 continuous per-query reciprocal ranks drawn from `{1, 1/2, 1/3, …, 0}`, not a count of successes, so a Wilson interval — which inverts the score test for a binomial proportion — has no `n` and no `p̂` to stand on and would publish bounds belonging to a different quantity. Resampling queries with replacement makes no distributional assumption about a per-query score and is the standard treatment for a rank-based mean | Every evaluation run; gate at P1 release |
...
| Identity-resolution merge precision | ≥ 0.95 as a **point estimate**, published with a 95% interval whose method and sidedness are named. The **rule of three** applies only at zero observed false merges; where one or more is observed the interval is a one-sided **exact binomial (Clopper–Pearson)** bound. The denominator is the number of merges made among labeled pairs of the estimation stratum — not the labeled-set size and not the run's total merge count — and where it falls below 30 the run discloses that the rule-of-three approximation is outside its reliable range | A wrong merge corrupts the log silently; this is the primary quality metric. **The target is the point estimate, not the interval's lower bound.** Read as a bound it is unreachable at this sample size: the rule of three gives 3/n, so even at zero errors with all 40 pairs merged the 95% lower bound is 0.925, and a 0.05 bound needs at least 60 merges. Naming only the rule of three also left the criterion unmeasurable in the one case it was written for, since a single false merge makes it inapplicable — the estimator per branch follows {SAD:ADR-0022}, interval method is selected per estimator | P1 release |
| Identity-resolution recall | ≥ 0.80 as a **point estimate**, published with a Wilson 95% interval and its denominator, explicitly secondary to precision | Missed links are visible and recoverable; the tradeoff is a stated design choice. **The target is the point estimate, not the interval's lower bound**, on the same arithmetic as precision above: at roughly 20 true pairs in the estimation stratum a 95% Wilson interval around 0.80 spans approximately 0.58–0.92, so the bound reading is equally unreachable. Wilson is admissible because recall is a proportion of hit-or-miss true pairs, per {SAD:ADR-0022} | P1 release |
...
| 80% prediction-interval coverage | Empirical coverage within 73–87% | Calibration is the core claim. The band was derived against an assumed ~120 uncensored events; E007 realized ~44, so the band is no longer the sampling error this split supports. It stands unchanged and unmeasurable rather than being widened to fit — see Assumptions | P1 release |
...
- Approximately 120 uncensored delivery events were assumed available after held-out splitting, bounding calibration precision to roughly ±4 percentage points. **Falsified by E007**: a 0.25 held-out split of the delivered 200-line history realizes approximately 44, widening the same one-standard-error bound to roughly ±6 percentage points. The 80% coverage band in Success Metrics is deliberately left unchanged — a band may only move by pre-registration, and this count became known only after the result it judges.

  **The figure was never reachable, and that is a different failure from a shortfall.** This assumption is a *held-out* count, since the ±4 points it claims is one standard error at 80% coverage on n = 120 and coverage is measured on held-out data. Delivering 120 held-out events at a 0.25 fraction requires roughly 480 uncensored events before the split. E005's FR-010 read the same 120 as the *training* remainder and derived a pre-split floor of `120 ÷ (1 − 0.25) = 160`, which at that fraction yields about 40 held-out events — and E007 realized 44. The dataset met every threshold asked of it. What no epic could see was that the thresholds were derived on the wrong side of the split: E005 checked its floor against its own derivation, E007 measured a result against this document, and the misreading sat between them where neither one's gate was looking. Corrected in E005's FR-010 rather than by moving this figure, because ~480 uncensored events needs roughly three times the line count the corpus scale permits — so the honest record is that this document's calibration precision is not purchasable at the registered dataset size, not that a generator underdelivered.
...
- **Sample sizes bound what can be claimed.** With 50 evaluation queries, 40 labeled pairs, and roughly 44 uncensored events — 120 were assumed; see Assumptions — confidence intervals are wide. Targets set above what the samples support would read as overfit. Mitigation: every metric published with its interval, and each interval computed by a method valid for the estimator it bounds — a proportion takes Wilson, a mean of per-query scores takes a bootstrap over queries. Publishing *an* interval was the original mitigation and it was not sufficient: an interval carrying the wrong method is more dangerous than none, because it presents as the very discipline it fails to exercise. Ablations and skill scores lead over absolute numbers. The forecast coverage band is the one target the realized count stopped supporting, and it is published as unmeasurable at this split rather than adjusted to fit.
...
- **Interval method is chosen per estimator, never per document.** An interval is a claim about a specific quantity's sampling behaviour, so the method has to be admissible for that quantity: a binomial proportion takes a Wilson score interval, and a mean of per-item scores takes a percentile bootstrap resampling the items. Both retrieval metrics were originally registered as "Wilson 95%" — correct for recall@5, which is a hit-or-miss count over 50 queries, and unfounded for MRR, which averages continuous reciprocal ranks and has no trial count to invert a score test on. The two rows now name their methods separately and state why they differ, because the failure was that one phrase covered two estimators and only one of them qualified. Neither threshold moved: this was never a claim about how good retrieval is, only about what the published interval around that claim actually measures. Any bootstrap interval publishes its resample count and its seed, since a resampled bound that cannot be regenerated is not reproducible evidence.
