...
- **SC-007** [US1]: Chunking the same document twice under the same input tuple produces identical boundaries and ordinals — measured by chunking in isolation and comparing, since an unchanged-tuple re-run writes no rows and so produces no second set to compare against.
...
- **SC-043** [US5]: At most one generation is active per document at any time, zero downstream reads return rows from more than one generation of the same document, and zero superseded generations' rows remain after a promotion completes.
